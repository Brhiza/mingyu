...
- 星盘排盘：西方占星完整排盘，包含太阳、月亮、上升星座与宫位、十大行星、逆行提示与主要相位分析；本命和行运相位仍输出角度偏差、容许度位置与紧密等级，双盘则穷举所选点对并完整返回实际夹角、精确角、偏差、采用容许度和双向落宫，不分级、不排序、不截断，也不把盘面几何事实包装成事件概率、匹配率或吉凶百分比；太阳返照采用粗定位加二分细化，并返回黄经残差、搜索过程和精度边界。
