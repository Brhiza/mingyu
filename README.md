...
- 八字排盘：输出四柱、十神、藏干、纳音、神煞、大运、流年、月令司权、旺衰条件与普通月令格局依据；出生时间必须通过输入约束后才进入排盘，不生成模糊时间候选盘。启用真太阳时时必须明确提供出生地经度与时区，不默认东八区；中国历史夏令时只在明确启用时校正。结果保存规范化公农历出生输入、真太阳时资料、完整神煞口径、中国夏令时开关和生成时间；公开证据、格式化、合盘、岁运与提示词只凭这些来源重建，不采信旧四柱、格局、神煞、大运、合盘或岁运派生字段，来源缺失、夹带、矛盾或非法时失败关闭。岁运入口只接受层级、运序及年月日原始选择，干支、十神、触发关系与提示词证据重新计算。底层只登记月令、司令、根气、藏透、成局及生克制化事实，不自动裁定整体身强身弱；专旺与从格在全局条件和来源闭合前失败关闭。未完成逐条来源、版本与适用边界校勘的自动用神、调候和病药规则同样失败关闭，明确返回取用待定，由 AI 结合完整盘面与问题继续复核。年限提示词可输出原局—大运—流年—流月—流日逐层合冲刑害破、伏吟、岁运并临与天克地冲证据；双盘可计算日主、日支、四柱交叉关系、双向十神及喜忌资料缺口，支持传统派、盲派、新派流派指引。
- 紫微斗数：以 `iztro` 原生能力输出十二宫、星曜、亮度、四化、三方四正及本命、大限、流年、流月、流日、流时，并随结果披露实际安星、闰月、分年、运限月份、小限年龄和晚子时口径；三合派、飞星派、四化派选项只改变解读侧重点，不改底层安星。启用真太阳时时必须明确提供出生地经度与时区，不默认东八区。结果的 `generation` 只保存传统时辰或真太阳时原始出生资料、完整排盘口径、生成时间、实际范围与轻量模式；公开结果和提示词会据此重建，不采信旧宫位、星曜、四化、运限、格局、证据或提示词。来源缺失、夹带、矛盾或非法时失败关闭。固定版本传统目录现登记 87 项，其中 55 条具备原文与可复算条件，32 项因原文含糊或依赖运限只登记边界、不伪造命中；原 84 条未校勘项目规则继续停用，目录外格局不作判断。
- 星盘排盘：西方占星完整排盘，包含太阳、月亮、上升星座与宫位、行星及扩展点、逆行提示与相位资料。本命对 24 个点位的 276 组无序点对逐一核验 10 种相位，行运对 10 个行运星体与全部 24 个本命点逐一核验五种主要相位；本命、行运、次限、太阳弧、太阳返照和双盘均完整返回当前定义与容许度内的命中项及实际几何量，不按派生强度筛选，不分级、不按强度或偏差排名删减、不截断，也不把盘面事实包装成事件概率、匹配率或吉凶百分比。生成结果会保存规范化出生输入、固定时区、IANA 时区、经纬度、真太阳时开关与生成时间；公开证据、提示词、摘要、推运和双盘只凭这些可信来源重建，旧星体、宫位、相位、摘要、光照、时区诊断和双盘结果均不采信，来源缺失或非法时失败关闭。行运取样采用目标地点当日历史时区；太阳返照采用粗定位加二分细化，并返回黄经残差、搜索过程和精度边界。
- 奇门遁甲：输出值符、值使、日干、时干及已校勘格局所在宫的位置索引，逐宫保留门星神干、已审核时家时旬空、宫间关系与反证，不用数字评分替代判断，也不自动指定用神、吉方、避方或应期。三奇六仪 81 种天地盘组合全部保留结构事实，11 项经《遁甲演义》逐条校勘的固定格自动进入传统格局；时家另开放伏干格、飞干格、岁格和六庚值符临丙格勃四项中性上下文结构，值符临地庚的天乙飞宫格与天盘庚临地值符旬首遁干的天乙伏宫格两项中性结构，乙奇到震三、丙奇到离九、丁奇到兑七三项中性升殿位置结构，三奇合开休生门并临太阴、九地、六合的真诈、重诈、休诈三项中性同宫结构，四书条件一致的天假、严格地假、鬼假三项中性奇仪门神同宫结构，以及六个固定时柱且值使门临地盘丁的玉女守门中性结构，不继承兵占、出行、百事、用途、主客、贵人、行动、方位或吉凶断语。伏干、飞干按 60 日柱 × 81 种天地盘组合穷举，岁格按 60 年干支 × 81 种组合穷举，格勃按 60 时柱 × 9 种值符宫地盘干穷举，天乙飞伏宫分别按 60 时柱 × 9 种地盘奇仪穷举，非甲申旬各 50 组命中，甲申旬因值符与六庚身份重合全部关闭；三奇升殿按 9 种天盘奇仪 × 9 宫共 81 组位置组合穷举，三诈及三项条件一致五假结构分别按 9 种天盘奇仪 × 8 门 × 8 神共 576 组同宫组合穷举，玉女守门按 60 时柱 × 8 值使门 × 9 地盘干共 4,320 组穷举，并以转盘、飞盘真实生成盘独立复算；天辅时按 10 日干 × 12 时支共 120 组验证全部失败关闭。甲日、甲年按六甲旬首遁干处理。升殿实际取用仍须结合吉门、门迫与入墓；三诈及三项条件一致五假结构缺少奇仪、门、神任一层时不命名；人假、物假、神假及地假太阴/六合扩展继续失败关闭。月格因“月干/月朔干”不一、时格因“本时干/仅三奇/庚值符管十时”不一、普通勃格因“丙临年月日时干/丙加值符庚”不一、天辅时因通行主表与《太白阴经》同条异说不一而继续失败关闭；月家、年家不外推这些时家结构。
  - 相佐只采用《遁甲演义》《奇门遁甲秘笈大全》《奇门旨归》《奇门法窍》不同范围的共同交集“值符临地盘丙”，按 60 时柱 × 9 种地盘奇仪穷举仅命中 60 组；地盘乙、丁不扩大命名。守户因值使临地丁、时干临地丁及守星/守门/守户异名冲突继续失败关闭；六个固定时柱且值使临地丁的玉女守门仍按独立已审核规则处理。
  - “符使同宫”在《奇门宝鉴御定》中说明值符、值使同起于旬首甲遁之宫、随后分别加临，不能据此把当前值符星和值使门同落一宫命名为独立吉格。两者当前同宫时仍分别保留值符落宫、值使落宫两个位置索引，但不生成“事必成”、力量集中、容易出结果、成功率或行动建议。
  - 九遁已逐书确认版本冲突：天遁至少有地盘丁、戊两本，地遁与人遁另有神盘、地盘干和门的扩展本，其余六遁在奇仪门神与落宫条件上冲突更大；九遁在 9 种天盘奇仪 × 9 种地盘奇仪 × 8 门 × 8 神 × 9 宫共 46,656 种组合中全部失败关闭。三奇得使另有固定六甲表、开休生值使门及两者并合三系定义，与三奇游六仪名称也不一致；按 3 种天盘三奇 × 6 种地盘六仪 × 8 门 × 8 神 × 9 宫 × 8 种值使门共 82,944 种组合全部失败关闭。两类规则只保留天盘干、地盘干、门、神、值使、值符和宫位原始事实，供 AI 在明确指定版本后继续核验。
  - 五合时只保留日干与时干的标准天干五合事实，不因《奇门宝鉴御定》所载名称及其与天辅时混称而自动命名；100 种日干时干组合全部失败关闭专项格名。天网与天网四张因六癸时、天盘癸落宫、值符临地癸及癸亥配特定局数等多套条件冲突，也不自动命名；60 时柱 × 9 落宫的天盘癸与值符临地癸两组条件均已穷举关闭，只向提示词提供完整时柱、阴阳遁局、天地盘干和值符旬首原始事实。
  - 三奇入墓、时干入墓与十干落宫入墓因原典混用天盘落宫、固定时柱和日干分组，且旧表漏掉日干前提并与所引原文表项不符，现已全部关闭自动名称、固定时柱标记和落宫标签。60 时柱及 9 种奇仪 × 9 宫均失败关闭，只保留完整日柱、时柱与九宫天地盘干，供 AI 明确版本和层级后复算。
  - 三奇受制在《奇门宝鉴御定》位置表与《遁甲演义》门、干、囚死复合条件之间层级不一；三奇会甲尚缺可核验的原文定义。3 种三奇 × 9 宫 × 8 门 × 9 地盘干共 1,944 组全部失败关闭两项名称，只保留原始盘面事实。
  - 六仪击刑按《遁甲符应经》《奇门遁甲统宗》《奇门旨归》《奇门法窍》共同的六组落宫表统一计算，并按 9 种时干所遁奇仪 × 9 宫共 81 组穷举；只在时家当前旬首六仪命中对应刑宫时保留中性位置事实，盘上其他六仪不扩大命中，月家、年家全部关闭，也不继承原典兵事、安营、出行或谋为断语。
  - 星门伏吟反吟只按时家实际落宫判断：八个外宫九星与八门回本位为伏吟、到本位对冲宫为反吟；按 9 星 × 9 宫及 8 门 × 9 宫穷举。因《遁甲符应经》与《奇门宝鉴御定》对六甲时是否直接算伏吟意见相反，不采用时辰捷径；天禽中五寄宫、月家和年家自动命名也全部关闭。
  - 门宫关系按 8 门 × 9 宫共 72 种组合穷举，只在门五行克所在宫五行时输出中性“门克宫”结构。因多书对门迫、宫迫名称存在倒置，迫制和义格的命名层级也不统一，不自动补成传统格名；时家、月家、年家均不继承主客、吉凶、行动或方位断语。
  - 旬空只在时家按完整时柱所属六旬输出固定两个时旬空地支，60 时柱逐项归入六组固定表；月家、年家没有取得把主动月柱或年柱直接作为旬空依据的闭合原文，全部关闭自动输出。时家按《奇门遁甲秘笈大全》日马四组表和完整日柱的日支计算日马，60 日柱穷举后申、寅、亥、巳各命中 15 次；日马只作中性落宫事实，不直接生成吉凶、行动、方位或应期。时马、月马、年马及月家、年家日马外推继续关闭。
- 住宅风水：产品统一入口，分层保留八宅命卦宅卦、东四西四分组、八宫传统标签，以及玄空飞星宅运盘；不自动生成方向宜避、命宅效果、布局建议或综合总分。可提供普通出生年月日+性别或直接命卦，并从固定山名、真北坐向度数、站在大门处面向屋内的测量三类山向来源中选择一种；玄空层须另有建造/起运年。统一结果的 `generation` 只保存居住人原始资料、单一山向来源、宅运年份与显式卦型口径，八宅、玄空、合参证据和提示词全部据此重建。真北坐向与门向测量接近二十四山或宅卦边界时并列全部候选盘，不把中心读数当作唯一坐向；即使缺少宅运年份，八宅层也会使用已提供的坐向度数并保留误差候选。旧命卦、宅卦、八宫、玄空三盘、替星、局型、候选坐向、证据和提示词均不采信；来源缺失、夹带、混用、双度数不严格相差 180°、附属字段脱离所属来源或字段非法时失败关闭。八命卦乘二十四山 192 盘、九运乘二十四山三种玄空口径 648 盘，以及 0 至 360° 的门向和真北坐向均已逐项验证审核重建等价。底层仍可单独调用八宅与玄空。
- 七政四余：输出七政四余十一星、命身十二宫、宿度、全部 55 组星对实际夹角、月相与出生时刻光照。七政、罗睺、计都和月孛采用现代天文位置，二十八宿用 28 颗真实距星在目标日期的黄经划界；紫炁保留《七政算内篇》古法均速模型，并明确区分位置精度层级。另按《张果星宗》分开提供天乙昼贵、玉堂夜贵及驿马、华盖、劫煞、咸池、孤辰、寡宿共 8 项生年起例目标支；春节与立春年界不一致时停止生成个人目标支，目标支不代表盘面命中或吉凶。生成结果保存规范化出生输入和生成时间；公开证据与提示词只凭这些可信来源完整重算，旧星体、星对、宿界、宫位、神煞、月相、光照、证据和提示词均不采信，来源缺失或非法时失败关闭。固定容许度吊照与简化庙旺表因缺少闭合原典依据而不自动判定。
- 时间型占卜不机械换算固定天数、周数或百分比：奇门只输出盘内相对节奏、触发条件与限制；小六壬只提供时间、干支、农历月日与时辰序号，固定底本未校定前不自动落宫、给出歌诀或推算应期。
...
- 真太阳时统一提升到 `mingyu-core/calendar`，线上提供 `/calendar/true-solar-time` 便捷换算接口；出生排盘必须明确提供当地钟表时区，不以东八区作为隐式默认值。
...
- 大六壬：天盘、四课、三传、月将、贵人、旬空、课体与问题范围选择；输出起盘及四课九宗门主版本与异说边界，并逐传给出相对日干的六亲、有方向生克、相邻关系、月令旺衰及条件化旬空资料。当前自动识别 35 条完成原文与条件校勘的课体，其中进茹、退茹按三传逐支顺逆相连命中，进间、退间按三传每次隔一位顺逆命中，回环要求三传全见于四课上神，赘婿只取日支临干受干克且以日支发用的多源交集，游子只取三传皆为辰戌丑未且月建天马发用的多源共同条件，泆女与狡童只取卯酉发用及初末传天后、六合次序的共同结构，天狱只取囚死墓发用且天罡辰临六壬专用日本表的共同结构，斩关只取魁罡辰戌临日干或日支发用且乘六合、青龙的严格交集；亨通只取三传逐级相生并最终生日干的双向递生结构，闭口只取旬尾临旬首发用，引从只取初末传所临地盘相邻夹日干寄宫或日支；九丑只取十个指定日柱、四仲时且天盘大吉丑临本日支，丑发用依正文，未发用则明确标记《订讹》分支，六仪、旬三奇只采用《六壬指南》明确的“发用”严格交集；非法或残缺课体上下文会失败关闭，官爵、龙首、龙战、芜淫等仍有版本条件分歧或缺少行年、旺衰层级等必要资料，继续关闭，未校勘的传统课体、神煞和断课目录不猜测命中。
- 小六壬：只保留可复核的起课时间、干支、农历月日、时辰及序号，并明确当前采用的闰月和换日历法口径。俗传六宫顺数与歌诀尚未取得可核验的固定底本、版本和页码，因此自动落宫、歌诀与扩展断法全部失败关闭；提示词要求 AI 先明确具体版本和完整规则后才能继续推算。
- 太乙神数：当前开放完成积年与阳遁七十二局立成校勘的年计，输出太乙、文昌、始击、计神、主客定算、将参与十六神盘，并提供计算链、主证、辅证、反证和方法限制组成的结构化证据。公开证据、提示词和摘要只信任生成时保存的原始公历年份，日期文本、干支、局数、宫位、定算、判断与旧证据全部重新计算；旧缓存缺少可信年份时失败关闭。月、日、时计须补齐节气时刻、章月、月法、日法、气应与小余等古籍历法链后再开放；Kintaiyi 的简式与分计不作为古籍真值。
...
- 生肖流年：按生肖年支与流年干支逐项计算值、冲、刑、害、破、三合、六合、三会和年干五行辅助关系；三合、三会只记录两支同属固定组，不表示完整成局。结果只以生肖年支和合法六十甲子为可信来源，公开证据与提示词会据此重建，不采信旧关系或旧提示词。结果输出计算链、主证、辅证、反证与信息量限制，不把五行生克改写成利弊，不把三合六合改写成现实贵人，也不生成综合吉凶等级或行动建议。
- 三山国王灵签：保留 1 至 92 的签号池、可重放抽签与掷筊轨迹。旧签谱缺少可定位到具体庙本、出版物或网页存档的来源，签题、签诗、典故与分类解签在完成逐签校勘前失败关闭，不进入公开结果或 AI 提示词。
...
- 黄历择日：支持搬家入宅、订婚结婚、开业启动、签约合作、出行赴任、就医手术、考试学习、安葬修坟、修造动土等事项；按 `tyme4ts` 原始宜忌、参与人刑冲破害、时辰与现实限制分成可用、条件和慎用候选，不设置或包装吉凶总分。公开证据、提示词、摘要与分页只信任原始事项、日期范围、参与人出生输入和生成时间，完整重算、分组并排序后再截取当前页；旧候选日、旧参与人八字、旧结论和旧证据均不采信，来源缺失或矛盾时失败关闭。
...
项目保留 1 至 92 的连续签号，用于抽取索引与随机轨迹重放。仓库早期整理的签题、签诗、典故和分类释义尚未找到可定位的固定底本，当前不再把这些文本标为祖庙原文或提供给 AI。后续只有取得具体庙本、出版物或可存档公开资料并完成逐签校勘后，才会重新开放相应文本。
