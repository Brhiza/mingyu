...
- 住宅风水：产品统一入口，分层保留八宅命卦宅卦、东四西四分组、八宫传统标签，以及玄空飞星宅运盘；不自动生成方向宜避、命宅效果、布局建议或综合总分。只需普通出生年月日+性别，或只填大门向屋内度数/山向，即可使用；玄空层须提供建造/起运年。测量接近二十四山或宅卦边界时并列全部候选盘，不把中心读数当作唯一坐向。八宅结果只保存出生资料或直接命卦，以及固定坐山或门向测量作为可信来源；公开证据、提示词、API、MCP 与住宅风水组合入口均由这些来源重建，旧命卦、宅卦、八宫、候选坐向、证据和提示词不采信，来源缺失、夹带、混用或非法时失败关闭。底层仍可单独调用八宅与玄空。
