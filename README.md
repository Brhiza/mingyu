...
- 奇门遁甲：输出值符、值使、日干、时干对应的用神宫候选，逐宫保留门星神干、空亡、马星、格局、宫间作用、反证及方位与时间触发条件，不用数字评分替代判断。三奇六仪 81 种天地盘组合全部保留结构事实，仅 11 项经《遁甲演义》逐条校勘的固定格自动进入传统格局。
...
- 奇门遁甲：时家奇门默认转盘法，可通过 API 参数请求飞盘法；定局默认拆补法，也可请求置闰法。包含天地人神四盘、值符值使、格局标签（含入墓、击刑、伏吟反吟、门迫等）、节令背景、已校勘组合与宫位洞察。八门余气固定采用《奇门遁甲统宗》五态月令版，十干迫制只保留奇仪落宫受克事实；通用入口不自动套用出军交战、星宫门宫主客、迷路、下营等缺少专项情境或存在版本冲突的规则，原始九宫、奇仪、星门神事实继续完整提供。
