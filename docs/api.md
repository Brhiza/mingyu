...
- 流年星盘中的太阳返照在连续 UTC 时间轴上先按 2 小时步长寻找太阳黄经过零区间，再用二分法细化，避免夏令时跳时和回拨破坏求根。默认以出生地生成返照盘，返回返照时刻、十大行星、四轴、十二宫、盘内相位及对本命触发；同时保留黄经残差、迭代次数、星历来源和精度边界。`scopeEvidence.solarReturnPeriods` 列出与目标日历年相交的返照有效区间，每项包含当地与 UTC 起止时刻、参考日有效标记和完整返照证据；原有 `solarReturnEvidence` 继续表示目标年份对应的一次返照。
- 次限推进按出生 UTC 瞬间的一岁一日映射计算日、月、水、金、火等推进点；太阳弧按推进太阳与本命太阳的弧度平移主要行星和四轴。两者分别使用窄容许度，并返回推进点与对本命相位的结构化证据。`yearly` 使用年中采样，`full` 使用请求中的基准日期，实足年龄按 365.2425 日/年换算。
