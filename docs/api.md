...
奇门排盘结果会包含 `seasonality` 和 `patternCombos`：前者给出节气三元、节气五行、历法八相、日月黄经月相证据、建除十二神和四柱干支互动，并保留历法八相与天文八分法是否一致；后者只保留固定文献条件可以闭合的门宫、主客、方位与时机组合规则。经典格、空亡、门迫、伏吟反吟和驿马等基础事实分别保留在原字段中，不再由项目二次拼成“吉凶混杂”“吉格逢空”或叠加吉凶等级。三奇六仪 81 种天地盘组合全部保留天干、五行生克与标准五合结构，只有《遁甲演义》卷一、卷二可逐条闭环的 11 项固定格会作为固定传统命中，其余 70 项不能单凭二元组合补造格名或现实断语。时家另按完整上下文识别伏干格、飞干格、岁格和六庚值符临丙格勃四项中性结构；甲日、甲年按完整干支确定所遁六仪，格勃还必须同时满足时柱属甲申旬、值符实际携庚且值符宫临地盘丙。月格、时格与普通勃格因原典版本条件冲突继续失败关闭；月家、年家不套用四项时家规则，也不输出原典兵占、出行、百事或主客利弊断语。提示词接口会把这些字段写入证据区，方便 AI 解读时引用。直接排盘接口可传 `detailMode: "compact"` 获取轻量结构；轻量结构只保留核心盘面、方位和少量关键组合，并返回完整数量，适合上游 AI 代理按需拆成多次请求。
