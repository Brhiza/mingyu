...
| `POST /bazi-ziwei/prompt`                    | 八字紫微分别计算并生成事实核对提示词                                           |
...
| `POST /metaphysics/zodiac/calculate`         | 从可信生肖年支和六十甲子重建值冲固定刑害破、六合支对、三合三会成员及结构化证据 |
...
| `POST /metaphysics/qizheng/prompt`           | 从可信出生来源重建七政四余盘并生成 AI 解读提示词                               |
...
1. 用户提供了完整出生信息，并希望同时核对八字与紫微资料时，可用 `POST /bazi-ziwei/prompt`。接口分别列出两套体系的可复算事实与资料边界，不以八字待定项补成紫微结论，也不以紫微位置事实补成八字旺衰、用神或喜忌。
...
| 查六十甲子、纳音、藏干和关系资料   | `POST /foundation/ganzhi`                    | `ganZhi`，如“甲子”                                                                                                                                                  | 返回单支固定资料与组成员原始表，不表示具体盘面的关系已经成立                                         |
...
| 同时核对八字与紫微出生盘资料       | `POST /bazi-ziwei/prompt`                    | `baziPromptTopic`、`ziweiPromptTopic` 按核对范围填写，`promptScope: "full"` 或 `"origin"`                                                                           | 分别返回两套体系的可复算事实，不自动生成统一现实结论                                                 |
| 同时核对某年或当前阶段资料         | `POST /bazi-ziwei/prompt`                    | `promptScope: "yearly"`，主题按所需资料范围填写                                                                                                                     | 八字列出已计算岁运触发，紫微列出流年落宫与四化位置；相似表述不等于互相验证                           |
...
| 住宅风水（八宅+玄空）              | `POST /metaphysics/residential/prompt`       | 山向或居住人至少一项：`birthYear`+`gender`/`mingGua`；山名、真北坐向度数、门向测量三类来源选一，可选 `year` 建造/起运年                                             | 统一入口；可只做人宅、只做宅运或两者合参；不给综合吉凶总分                                           |
...
| 生肖流年固定关系                   | `POST /metaphysics/zodiac/prompt`            | `zodiac`、`year` 或 `yearGanZhi`                                                                                                                                    | 生肖可传“鼠”或“子”；寅巳申、丑戌未任意二支不命名刑太岁，三合三会只记录同组成员，不等于完整关系或成局 |
...
住宅风水结果的 `generation` 只保存居住人出生资料或直接命卦、山名/真北坐向度数/门向测量之一、宅运年份与显式卦型口径。公开 API、MCP、八宅层、玄空层、合参证据和提示词都先从该来源重建；旧两层盘面、候选、证据和提示词不会进入下一步。山向来源混用、不完整出生资料、无山向却夹带宅运年份、附属测量字段脱离所属来源、磁北缺少磁偏角或非法字段会直接返回参数错误。八命卦乘二十四山 192 组、九运乘二十四山三种口径 648 组，以及 0 至 360 度门向和真北朝向均已逐项验证。
...
返回值中的六合、冲、害、破是单支固定对象；三合、三会、三刑字段只是所属组的原始成员资料。大六壬定向刑序只供该体系使用；公共半合、拱局入口因柱位、冲破、透干和版本条件不完整而失败关闭。调用方必须结合另一干支、柱位和具体体系再判断关系是否成立。
...
curl -X POST https://aov.cc/api/v1/bazi/calculate \
  -H "Content-Type: application/json" \
  -d '{"gender":"male","year":1990,"month":5,"day":15,"timeIndex":1,"dateType":"solar","shenShaVariants":{"kongWangBasis":"day-and-year","yangRenMode":"include-yin-ren"}}'
```
...
紫微返回的 `generation` 只保存传统时辰或真太阳时原始出生资料、完整排盘口径、生成时间、实际范围与轻量模式。公开排盘、摘要、单盘提示词和八字紫微合参会从该来源重新生成宫位、星曜、四化、运限、格局和真太阳时证据，不采信旧派生盘面或旧提示词；来源缺失、夹带、矛盾、重复范围或非法时直接拒绝。真太阳时来源保留原始精准时分、经度、时区与中国历史夏令时开关，不只保存校正后的日期和时辰。完整与轻量结果都会返回 `generation`。
...
八字紫微提示词用于同时取得两套体系的可复算事实与资料边界，`promptScope` 同样支持 `full`；当前不自动生成跨体系因果映射或统一现实结论：
...
奇门排盘结果会包含 `seasonality` 和 `patternCombos`：前者只给出实际节气与天文交节证据、月令五行、日干旺相休囚死状态、历法八相与日月黄经月相证据、建除名称和四柱干支互动；上中下元只采用 `timeInfo.epoch` 中按甲己符头或所选置闰法形成的正式定局结果，不再把交节后自然日机械分段。历法蛾眉月、盈凸月等也不再粗并成上弦或满月，建除不附带脱离具体事项的固定吉凶和宜忌建议。`patternCombos` 只保留固定文献条件可以闭合的中性组合事实。经典格、空亡、门迫、伏吟反吟和驿马等基础事实分别保留在原字段中，不再由项目二次拼成“吉凶混杂”“吉格逢空”或叠加吉凶等级。三奇六仪 81 种天地盘组合全部保留天干、五行生克与标准五合结构，只有《遁甲演义》卷一、卷二可逐条闭环的 11 项固定格会作为固定传统命中，其余 70 项不能单凭二元组合补造格名或现实断语。时家另按完整上下文识别伏干格、飞干格、岁格和六庚值符临丙格勃四项中性结构，并识别值符临地庚的天乙飞宫格与天盘庚临地值符旬首遁干的天乙伏宫格；甲申旬值符本身即六庚，因身份重合不自动命名。另按天盘乙到震三、丙到离九、丁到兑七识别三项中性升殿位置结构，在天盘三奇、开休生门及太阴/九地/六合三层同宫时分别识别中性真诈/重诈/休诈，并只按四书共同条件识别天假、严格地假、鬼假三项中性奇仪门神同宫结构；甲日、甲年按完整干支确定所遁六仪，格勃还必须同时满足时柱属甲申旬、值符实际携庚且值符宫临地盘丙。升殿只证明三奇落宫，实际取用仍须结合吉门、门迫与入墓；三诈及三项条件一致五假结构只证明奇仪、门、神完整同宫，缺少任一层或只在地盘见相应干时不命名。人假、物假、神假及地假太阴/六合扩展因版本冲突继续失败关闭；月格、时格与普通勃格同样继续关闭；月家、年家不套用这些时家规则，也不输出原典兵事、主客、贵人、吉凶、行动或方位断语。提示词接口会把这些字段写入证据区，方便 AI 解读时引用。直接排盘接口可传 `detailMode: "compact"` 获取轻量结构；轻量结构只保留核心盘面、方位和少量关键组合，并返回完整数量，适合上游 AI 代理按需拆成多次请求。

相佐只开放时家当前值符临地盘丙这一四书共同交集，并只作为中性结构事实；地盘乙、丁不扩大命名。守户在值使临地丁、时干临地丁及守星/守门/守户异名之间条件未统一，接口不自动命名；六个固定时柱且值使临地丁的玉女守门仍按独立已审核规则输出。

“符使同宫”在《奇门宝鉴御定》中属于值符、值使同起于旬首甲遁宫、随后分别加临的排布机制，不是“当前值符星和值使门同落一宫即为吉格”的可靠定义。接口遇到两者当前同宫时会分别保留 `值符落宫`、`值使落宫` 两个位置索引，但不输出符使同宫格名，也不生成事必成、力量集中、容易出结果、吉凶、成功率或行动建议。

四柱干支背景只输出固定六合、六冲、相害、子卯相刑、自刑、天干五合与已登记天干冲，以及三合或三刑的完整三支齐见结构。天干五合、地支六合只登记固定配对、传统化气五行资料及月令状态、透根、位置、冲克和相同配对候选等原始条件；旧固定月份表及“成化、不化、合绊、破合、争合”自动裁决失败关闭，经典外格入口也不再从五合、月份和完整支局自动命名五种化气格。三支齐见不等于成局或合化；半合、拱局及寅巳申、丑戌未任意二支相刑因所需相邻、冲破、透干或版本条件未闭合而失败关闭，原始四柱仍完整保留供 AI 明确口径后继续推算。

九遁与三奇得使已经完成版本冲突审核，但没有开放自动命名。九遁在天盘奇仪、地盘奇仪、八门、八神与九宫组成的 46,656 种候选组合中全部失败关闭；三奇得使及三奇游六仪在天盘三奇、地盘六仪、八门、八神、九宫与值使门组成的 82,944 种候选组合中全部失败关闭。接口仍返回天盘干、地盘干、门、神、值使、值符和宫位事实，AI 只有在明确指定原典版本后才可继续核验，不得把任一孤本条件当成既定格名、吉凶、用途或现实结果。
...
- 紫微结果、摘要、单盘提示词与八字紫微合参只消费 `generation` 中的可信来源；旧宫位、星曜、四化、运限、格局、证据、真太阳时校正结果和提示词不会进入下一步计算。来源结构已穷举性别、公农历、13 时辰、两套算法及四组分界口径共 1,664 种组合；盘面结构另有 624 组跨世纪组合和 1,440 组独立反算样本。
...
- 八字紫微资料核对接口为 `POST /bazi-ziwei/prompt`，使用同一份出生信息，同时计算八字和紫微，默认返回 `data.resultSummary.bazi`、`data.resultSummary.ziwei` 和 `data.prompt`；传 `responseMode: "full"` 可返回完整双盘。该接口使用 `baziPromptTopic`、`ziweiPromptTopic`、`promptScope` 区分两套体系的资料范围，只并列已成立事实，不把相似表述当作互相验证或可信度提升。
...
- 八字 `shenShaVariants` 用于请求神煞争议口径；不传时使用默认主流口径：空亡只按日柱旬空、羊刃只取阳干帝旺。可选值：`kongWangBasis` 为 `day` 或 `day-and-year`；`yangRenMode` 为 `yang-stems-only` 或 `include-yin-ren`。神煞只记录名称与柱位，不自动分类吉凶或现实事项；童子煞、十灵日、六秀日因依据尚不能逐条复核，正式入口不再生成。
- 紫微 `school` 支持 `sanhe`、`feixing`、`sihua` 资料口径标签。标签不改变底层安星算法，也不提供完整解释规则；不得仅凭标签补造宫位主轴、宫干飞化、吉凶、事件、应期或建议。本次实际采用的 `iztro` 算法、闰月、分年、运限月份、小限年龄和晚子时口径以 `calculationConfig` / `payloadByScope.*.calculation_config` 为准。
...
- 小六壬 `xiaoliurenMethod` 当前仅支持 `time`（默认）；可用 `customDate` 指定起课时间。公开结果只提供重新换算的时间、干支、农历月日、时辰序号和历法口径。俗传顺数、落宫、六宫歌诀以及数字、随机和流派扩展因固定底本、具体版本与页码未闭合而失败关闭；提示词要求先明确采用的版本和完整规则后再继续推算。
...
- 六爻排盘结果的 `evidenceAnalysis` 返回分层用神候选、本卦与伏神爻位、原神忌神仇神作用链、月日和动变支持、空破墓退反证及病药触发条件。公开证据、提示词和摘要统一从可信原始输入重建：时间法只认时间戳，手工法只认时间戳与六个原始爻值，实投三钱只认时间戳与六组铜钱记录，随机三钱只认时间戳与恰好 18 个可重放随机样本；随机种子存在时逐样本复算。卦名、纳甲、世应、六亲、六神、旺衰、伏神、组合关系和旧证据均不采信；来源缺失、夹带、矛盾或轨迹无法复算时明确拒绝。作用链的 `effectFacts` 按每个用神、原神、忌神引用返回真实活动状态、有力/有气条件和无力/无根条件；`godInteractionFacts` 以引用键和逐步五行关系返回直接生克、忌原接续相生、原神受制得助及忌神受制得助路径。静爻旺相不等于已经作用，原忌同动与忌仇同动只按重算后的明动或暗动成立，条件和路径并见时不按数量裁定有效无效。`sanheFormations` 分别记录三爻齐动、两动一静、初三/四六爻动变、日月补局和虚一待用，并保留参与爻、缺支、空破墓与静爻待值状态；同一爻的本支、变支不能冒充日月补局所需的两个活动爻。结构事实通过 `sanheFormationKey`、`sanhePattern`、`sanheStatus`、`sanheRole` 和 `referenceKeys` 关联三合与用神链；静爻逢月日合记合起，明动或暗动逢合记合绊，两活动爻相合记合好，动化六合另记化扶。爻支与单个月建或日辰的相刑只保留子卯固定支对和辰午酉亥重复自刑；寅巳申、丑戌未须在卦内三支齐见并满足发动条件后另列完整三刑结构。变爻只通过本位动爻形成回头生克冲、进退空墓破等条件，不跨位充当独立原忌仇神；应期事实用 `role`、`effect` 和 `referenceKeys` 区分用神受制待解、原神助用待验、忌神制化待辨及边界。提示词会按 `liuyaoTemplate` 重新选择对应主题候选；整卦冲合、三合须结合事项、用忌、世爻和旺衰辨向，取用未闭合时不套空破合冲公式，也不输出吉凶总分、成功率或固定日期。主互变与审核重建按 64 卦 × 6 动爻逐案验证等价。
...
- 奇门排盘结果的 `evidenceAnalysis` 返回值符、值使、日干、时干对应的用神宫候选，以及逐宫门、星、神、天地盘干、已审核时家时旬空、时家日马、格局、宫间生克、反证、方位条件和时间触发条件。时家日马按完整日柱的日支与四组固定表计算，只作中性落宫事实；60 日柱穷举后四个目标支各命中 15 次。月家、年家旬空以及时马、月马、年马和年月家日马外推继续关闭；候选不等于已经按具体问题选定用神，核心结果、公开 API、MCP 与提示词均不返回内部宫位、格局或方位排序分数，也不机械换算绝对日期。
...
- 黄历择日结果的 `evidenceAnalysis` 会把当前返回范围内的日期分成可用、条件和慎用候选，逐日列出事项宜忌、建除神煞、参与人冲、固定刑、害、破、方向限制、可用时辰与现实约束。参与人双支相刑只保留子卯固定支对和辰午酉亥重复自刑，寅巳申、丑戌未任意二支失败关闭；这些关系只作参考，不自动改变候选分组。公开证据、提示词和摘要只信任 `generation` 保存的原始事项、日期范围、参与人出生资料及生成时间；顶层事项、日期、候选日、参与人派生八字、旧证据与旧时间戳全部忽略。旧结果缺少原始生成资料、参与人编号重复、输入矛盾或分页范围非法时明确拒绝。核心结果、公开 API、MCP 与提示词均不返回内部日期或时辰排序分数，也不把排序解释成成功率。
- 雷诺曼 `spreadType` 支持 `single`、`three`、`five`、`relationship`、`decision`、`nine`、`element`、`grandTableau`，不传时使用 `single`。
- 星盘需要 `year`、`month`、`day`、`hour`、`minute`、`latitude`、`longitude`，并至少提供 `timezone` 或 `timeZoneId`。历史日期及实行夏令时的地区推荐使用 IANA 时区；秋季回拨的一时两刻会保留候选和警告，春季跳时中不存在的当地时刻会被拒绝。可传 `useTrueSolarTime` 附带真太阳时参考证据，但现代星历始终采用民用出生时间对应的真实 UTC 瞬间。结果的 `generation` 保存规范化出生输入和生成时间；公开证据、提示词、摘要、本命/行运范围、次限、太阳弧和太阳返照只凭该来源重建。旧结果中的星体、宫位、相位、光照、摘要、时区诊断和顶层时间均不作为可信输入，缺少合法 `generation` 时明确拒绝。
...
- 西占双盘使用 `person1`、`person2` 包裹双方星盘参数。排盘结果穷举全部所选点对，完整返回所有命中主要跨盘相位的实际夹角、精确角、偏差、采用容许度及全部双向跨盘落宫；核心结果、公开 API、MCP 与提示词均不分级、不排序、不截断，也不派生关系强弱、匹配率或吉凶比例。双盘 `generation` 保存双方本命原始来源、完整点位选择、五种主要相位实际容许度、落宫开关与生成时间；审核重建和提示词只消费这些来源，旧本命派生盘面、旧跨盘相位、旧落宫和外部时间均会被忽略，来源缺失或参数非法时失败关闭。
