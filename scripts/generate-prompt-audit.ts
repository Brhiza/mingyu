...
const REQUIRED_SAMPLE_FIELDS: RequiredSampleFields[] = [
  ...
  {
    sampleName: '住宅风水',
    requiredFields: ['【当前时间】', '【问题】', '【任务】', '【传统依据】', '合参要点'],
  },
  {
    sampleName: '玄空飞星',
    requiredFields: [
...
function buildPromptMarkdown(samples: PromptSample[]) {
  const lines = [
    ...
    '',
    '说明：本文件由项目本地函数真实生成，覆盖八字排盘、紫微斗数、星盘、六爻、梅花易数、奇门遁甲、大六壬、塔罗牌、三山国王灵签、择日、八宅风水、住宅风水、玄空飞星、太乙神数。七政四余完整盘在传统坐标链校勘完成前暂停输出，因此不生成可交给模型解读的样本。八字、紫微斗数、星盘测试资料取自比赛原题公开出生信息，未读取正确答案文件。',
    '',
    '## 审计原则',
    '',
