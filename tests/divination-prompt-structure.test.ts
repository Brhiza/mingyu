...
test('奇门提示词会输出值符值使、旬空、日马边界和格局资料', () => {
  ...
  assert.match(
    prompt,
    /旬空与日马边界：旬空[子丑寅卯辰巳午未申酉戌亥]空落\S+宫、[子丑寅卯辰巳午未申酉戌亥]空落\S+宫；日马[子丑寅卯辰巳午未申酉戌亥]落\S+宫（仅作位置事实）；日马不直接代表快慢、行动、吉凶或方位结论/,
  );
...
test('大六壬提示词在四项类神资料不全时只保留可复算课传事实', () => {
  const prompt = buildDivinationPrompt(
    'liuren',
    '这件事接下来该怎么推进？',
    createData('liuren'),
  ...
  assert.match(
    prompt,
    /【任务】\n先检查调用方是否同时明确提供具体类神底本版本、事项类别与参与者角色、完整类神取用规则和已指定类神对象/,
  );
  assert.match(prompt, /问题文字、问题范围标签、日干、日支、初传、天将或神煞不能替代这四项资料/);
  assert.match(
    prompt,
    /【输出要求】\n先核对具体类神底本版本、事项类别与参与者角色、完整类神取用规则和已指定类神对象是否同时齐全/,
  );
  assert.match(prompt, /不生成现实演变、成败、时机或行动建议/);
  assert.doesNotMatch(prompt, /直接说明演变、卡点与下一步|先回答【问题】.*时机条件和行动建议/);
});
