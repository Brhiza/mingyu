...
const getPrimaryRuntime = createZiweiRuntimeFixture(
...
const getPartnerRuntime = createZiweiRuntimeFixture(
  buildZiweiChartInput({
    name: '乙',
    gender: 'female',
    dateType: 'solar',
...
const getZhongzhouRuntime = createZiweiRuntimeFixture(
  buildZiweiChartInput({
    name: '',
    gender: 'female',
    dateType: 'solar',
...
test('紫微 MCP 返回结果应为可 JSON 序列化的纯数据', async () => {
  const runtime = await getPrimaryRuntime();
...
test('紫微 MCP 支持中州派底层安星口径', async () => {
  const runtime = await getZhongzhouRuntime();
  assert.equal(runtime.payloadByScope.origin.calculation_config.algorithm, 'zhongzhou');
});

test('紫微合盘主题只作为关系范围，不再注入固定问题与任务口径', async () => {
  const firstRuntime = await getPrimaryRuntime();
  const secondRuntime = await getPartnerRuntime();
...
test('紫微合盘自定义问题不拼接关系预设，只保留通用短框架', async () => {
  const firstRuntime = await getPrimaryRuntime();
  const secondRuntime = await getPartnerRuntime();
