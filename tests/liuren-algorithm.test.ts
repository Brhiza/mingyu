  ...
  assert.ok(
    result.shenShaFacts?.every(
      (item) =>
        ...
        item.limitations.some((limitation) => limitation.includes('一百三十项可复算神煞规则')),
    ),
  );
});
...
test('大六壬已登记神煞应覆盖十二月建与六十日柱固定表', () => {
  ...
  const finalAuditedMonthTargets: Record<string, readonly string[]> = {
    寅: ['未', '亥', '卯', '酉', '戌', '丑', '卯', '寅', '辰', '辰', '辰'],
    卯: ['辰', '午', '卯', '酉', '戌', '寅', '子', '卯', '辰', '巳', '戌'],
    辰: ['丑', '丑', '卯', '酉', '戌', '卯', '酉', '辰', '辰', '午', '丑'],
    巳: ['戌', '申', '午', '子', '未', '辰', '午', '巳', '未', '未', '未'],
    午: ['未', '卯', '午', '子', '未', '巳', '卯', '午', '未', '申', '辰'],
    未: ['辰', '戌', '午', '子', '未', '午', '子', '未', '未', '酉', '戌'],
    申: ['丑', '巳', '酉', '卯', '辰', '未', '酉', '申', '戌', '戌', '丑'],
    酉: ['戌', '子', '酉', '卯', '辰', '申', '午', '酉', '戌', '亥', '未'],
    戌: ['未', '未', '酉', '卯', '辰', '酉', '卯', '戌', '戌', '子', '辰'],
    亥: ['辰', '寅', '子', '午', '丑', '戌', '子', '亥', '丑', '丑', '戌'],
    子: ['丑', '酉', '子', '午', '丑', '亥', '酉', '子', '丑', '寅', '丑'],
    丑: ['戌', '辰', '子', '午', '丑', '子', '午', '丑', '丑', '卯', '未'],
  };
  ...
  for (let dayOffset = 0; dayOffset < 60; dayOffset += 1) {
    ...
    assert.equal(
      shenShaFacts.length,
      128 + Number(hasTianHe) + Number(hasTianShe),
      `${result.ganzhi.day}应有一百二十八项固定神煞及条件性天合、天赦`,
    );
