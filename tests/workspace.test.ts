...
test('工作区应为每个排盘和占问工具提供独立入口', () => {
  ...
  assert.equal(buildWorkspaceFeaturePath('calendar'), '/calendar');
  assert.equal(isCalendarWorkspaceId('calendar'), true);
  assert.equal(WORKSPACE_FEATURES.find((item) => item.id === 'calendar')?.label, '干支日历');
