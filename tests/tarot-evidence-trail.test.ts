...
import { drawTarotSpread } from '../packages/core/src/divination/tarot.ts';
import { validateEvidenceItem } from '../packages/core/src/shared/evidence.ts';

// --- 1. drawTarotSpread 结果附带 evidenceTrail ---
test('tarot evidenceTrail：drawTarotSpread 结果包含完整证据链', () => {
  const data = drawTarotSpread('three', { seed: 'test-seed' });
  ...
  assert.ok(data.evidenceTrail!.summary.includes('塔罗抽牌证据链'), 'summary 应标记塔罗抽牌');
  assert.ok(data.evidenceTrail!.overallConfidence, '应有整体置信度');
});

// --- 2. 四字段契约完整性 ---
test('tarot evidenceTrail：每条证据满足四字段契约', () => {
  const data = drawTarotSpread('three', { seed: 'test-seed' });
...
// --- 3. 覆盖环节断言：基础 + 洗牌 + 牌面 ---
test('tarot evidenceTrail：覆盖抽牌基础、洗牌、牌面环节', () => {
  const data = drawTarotSpread('three', { seed: 'test-seed' });
...
// --- 4. 抽定牌数与结果一致 ---
test('tarot evidenceTrail：牌面输出与结果一致', () => {
  const data = drawTarotSpread('three', { seed: 'test-seed' });
...
// --- 5. 证据深度排序 ---
test('tarot evidenceTrail：证据按 depth 升序排列', () => {
  const data = drawTarotSpread('three', { seed: 'test-seed' });
...
// --- 6. 不同牌阵产生独立证据链 ---
test('tarot evidenceTrail：不同牌阵证据链独立生成', () => {
  const single = drawTarotSpread('single', { seed: 'a' });
  const three = drawTarotSpread('three', { seed: 'a' });
  assert.notEqual(single.evidenceTrail!.summary, three.evidenceTrail!.summary, '不同牌阵 summary 应不同');
  assert.ok(three.evidenceTrail!.summary.includes('3'), 'three 阵 summary 应含张数');
});
