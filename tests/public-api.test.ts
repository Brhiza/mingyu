...
test('公开 API OpenAPI 文档应标明占卜提示词接口返回摘要', async () => {
  ...
  assert.equal(body.data.components.schemas.ShenShaVariants.properties.tongZiScope, undefined);
});
...
test('公开 API 应支持八字排盘', async () => {
  ...
  assert.equal(body.data.evidenceAnalysis.kinshipFacts.length, 9);
...
test('公开 API 不得静默接受已失败关闭的童子煞口径', async () => {
  const { response, body } = await callApi('bazi/calculate', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
  ...
  assert.equal(body.ok, false);
  assert.equal(body.error.code, 'BAD_REQUEST');
  assert.match(body.error.message, /童子煞规则尚无可逐条复核的依据/);
});
...
test('公开 API 雷诺曼接口应返回原始抽牌记录与牌义待校边界', async () => {
  ...
  assert.deepEqual(body.data.evidenceAnalysis.layoutFacts, []);
  assert.deepEqual(body.data.evidenceAnalysis.traditionalFacts, []);
  assert.equal(body.data.evidenceAnalysis.spreadCoverageFact.status, '完整');
  assert.equal(
  ...
  assert.equal(body.data.evidenceAnalysis.layoutCoverageFact.status, '结构缺失');
  ...
  assert.deepEqual(body.data.evidenceAnalysis.structuredLayoutFacts, []);
  ...
  assert.equal(body.data.evidenceAnalysis.evidence.title, '雷诺曼抽牌原始记录与牌义待校边界');
  ...
  assert.doesNotMatch(lenormandPromptResponse.body.data.prompt, /关键词：|牌义：|组合明细：/);
  assert.doesNotMatch(
    lenormandPromptResponse.body.data.prompt,
    /结构化证据|计算链|证据汇总|解释限制|解释边界/,
  );
  ...
  assert.ok(
    defaultResult.body.data.evidenceAnalysis.counterEvidenceFacts.every(
      (item: Record<string, unknown>) =>
        ...
        Array.isArray(item.sources) &&
        String(item.limitation).includes('不得把单项位置限制直接写成现实失败'),
    ),
  );
  assert.ok(
    defaultResult.body.data.evidenceAnalysis.timingFacts.every(
      (item: Record<string, unknown>) =>
        ...
        item.promptText &&
        Array.isArray(item.sources) &&
        String(item.limitation).includes('不生成相对节奏'),
    ),
  );
  assert.equal(
...
test('公开 API 奇门排盘支持轻量模式，便于调用方按需拆分请求', async () => {
  ...
  assert.equal('horseStar' in fullResult.body.data, false);
  ...
  assert.equal(compactResult.body.data.palaceInsights, undefined);
  assert.equal(compactResult.body.data.palaceInsightTotal, undefined);
  assert.ok(
  ...
  assert.equal('horseStar' in compactResult.body.data, false);
});
...
test('公开 API 梅花排盘与提示词应返回主互变体用推进证据', async () => {
  ...
  assert.equal(chart.body.data.evidenceAnalysis.objectContextFact.status, '资料不足');
  assert.deepEqual(chart.body.data.evidenceAnalysis.objectContextFact.availableContextFields, []);
  assert.deepEqual(
    chart.body.data.evidenceAnalysis.objectContextFact.missingContextFields,
    chart.body.data.evidenceAnalysis.objectContextFact.requiredContextFields,
  );
  ...
  const tenResponse = chart.body.data.evidenceAnalysis.tenResponseContextFact;
...
test('公开 API 六爻与大六壬提示词接口保留用户模板范围', async () => {
  ...
  const liurenTransmissionConventionFact =
    liurenChart.body.data.evidenceAnalysis.transmissionConventionFact;
  assert.equal(liurenTransmissionConventionFact.key, 'liuren:transmission-convention');
  assert.equal(liurenTransmissionConventionFact.status, '已登记版本边界');
  assert.equal(liurenTransmissionConventionFact.lessonRules.length, 4);
  assert.deepEqual(liurenTransmissionConventionFact.methodOrder, [
    '贼克',
    '比用',
    '涉害',
    '遥克',
  ...
  assert.equal(liurenTransmissionConventionFact.sheHaiRule.useZeBi, false);
  assert.match(liurenTransmissionConventionFact.sheHaiRule.depthRule, /实际受克深浅/);
  assert.match(liurenTransmissionConventionFact.remoteKeRule, /二、三、四课/);
  assert.ok(
    liurenTransmissionConventionFact.specialMethodRules.some(
      (item: { method: string; rule: string }) =>
        item.method === '八专' && item.rule.includes('癸丑'),
    ),
  );
  assert.ok(
    liurenTransmissionConventionFact.alternativeVersionFields.some((item: string) =>
      item.includes('择比'),
    ),
  );
  assert.match(liurenTransmissionConventionFact.limitation, /初传发用到中末传整体重排/);
  ...
  assert.equal(
    liurenChart.body.data.evidenceAnalysis.summaryFact.transmissionConventionFactCount,
    1,
  );
  ...
  const liurenFactKeys = new Set([
    ...
    liurenChart.body.data.evidenceAnalysis.transmissionConventionFact.key,
  ...
  assert.match(liuren.body.data.prompt, /四课取传口径：/);
...
test('公开 API 新增术数提示词应包含用户问题和统一章节', async () => {
  ...
  assert.match(
    body.data.prompt,
    /误差候选：寅山申向（艮宅、西四命、命宅分组异组）、甲山庚向（震宅、东四命、命宅分组同组）/,
  );
  ...
  assert.equal(
    body.data.result.evidenceAnalysis.summaryFact.sameLabelDirectionCount,
    body.data.result.evidenceAnalysis.sameLabelDirections.length,
  );
  assert.equal(
    body.data.result.evidenceAnalysis.summaryFact.differentLabelDirectionCount,
    body.data.result.evidenceAnalysis.differentLabelDirections.length,
  );
  assert.equal(
...
test('公开 API 生肖流年应返回关系矩阵证据而不使用综合吉凶定级', async () => {
  ...
  assert.match(prompt.body.data.prompt, /资料边界：/);
  ...
  assert.doesNotMatch(JSON.stringify(prompt.body.data), AUTOMATIC_ZODIAC_CONCLUSION);
});

test('公开 API 生肖流年应返回三会关系且不生成贵人、利弊或行动字段', async () => {
  const calculate = await callApi('metaphysics/zodiac/calculate', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
test('公开 API 住宅风水接口返回八宅与玄空分层事实', async () => {
  ...
  assert.equal('agreements' in body.data.result, false);
  assert.equal('advice' in body.data.result, false);
  assert.match(body.data.prompt, /【住宅风水排盘】/);
  assert.match(body.data.prompt, /资料与复核提示：/);
