...
test('奇门未审核符使、月格时格与普通勃格规则不得进入经典格局', () => {
  const contexts = [
    {
      jiuGongGe: [
  ...
  ['月格', '时格', '勃格', '地罗遮蔽', '相佐', '守户', '玉女守门', '天辅时', '五合时'].forEach(
    (name) => assert.doesNotMatch(serialized, new RegExp(name)),
  );
});
...
test('奇门跨年跨月跨时辰与两种排盘法只输出已审核格局事实', () => {
  ...
  const retiredPattern =
    /九遁|天遁|地遁|人遁|神遁|鬼遁|龙遁|虎遁|风遁|云遁|三奇得|三奇游六仪|三诈|真诈|重诈|休诈|人假|物假|神假|升殿|奇入墓|奇受制|三奇会甲|符使同宫|相佐|守户|月格|时格|勃格|地罗遮蔽|天辅时|五合时|玉女守门/;
