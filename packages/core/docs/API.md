...
**返回 `LiurenData`：** 含月将（中气换将）、昼夜贵人、天地盘、四课、三传（初传/中传/末传，含九宗门取传法）、已审核神煞、天将属性、13 条已校勘课体规则与旬空。课体只是可复算盘面事实，不直接生成吉凶结论。

### `getLiurenGuaTiFacts(context)` / `getLiurenTransmissionGuaTi(branches)`

识别已登记的课体事实或课体名称。三传必须恰好包含三个合法地支；四课一经提供就必须完整包含四课，课上神须为地支，课下位须为天干或地支；非法、残缺或超长输入会直接报错，不会按相似结构猜测。当前只开放 13 条完成原文与条件校勘的规则，不代表传统课体总目录已经穷尽。

课体命中边界已独立穷举全部 1,728 种三传排列；完整排盘另验证 17,280 组月将、占时、日柱、昼夜组合，以及加入十二太岁后的 207,360 个课体上下文。
