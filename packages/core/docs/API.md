...
**返回 `QimenData`：** 含完整排盘：定局数（拆补法）、值符值使、九宫格（天地人神四盘）、基础格局标签、经典格局（九遁/三奇得使等）、节令背景、已校勘组合规则、宫位洞察、方位吉凶、应期估算（庚格法）、马星落宫、旬空。
...
| `patternCombos` | `QimenPatternCombo[]?` | 固定文献条件可闭合的门宫、主客、方位与时机组合规则；不二次拼接基础事实或吉凶等级 |
