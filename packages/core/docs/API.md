...
**返回 `LiurenData`：** 含月将（中气换将）、昼夜贵人、天地盘、四课、三传（初传/中传/末传，含九宗门取传法）、已审核神煞、天将属性、25 条已校勘课体规则与旬空。课体只是可复算盘面事实，不直接生成吉凶结论。

### `getLiurenGuaTiFacts(context)` / `getLiurenTransmissionGuaTi(branches)`

识别已登记的课体事实或课体名称。三传必须恰好包含三个合法地支；四课一经提供就必须完整包含四课，课上神须为地支，课下位须为天干或地支；日柱、日干、日支、占时、大吉落地等可选上下文一经提供也须合法且不得互相矛盾，缺少九丑、天网、上门乱首、六仪、旬三奇等专项规则所需条件时不会猜测命中。非法、残缺或超长输入会直接报错。当前只开放 25 条完成原文与条件校勘的规则，不代表传统课体总目录已经穷尽。

课体命中边界已独立穷举全部 1,728 种三传排列，进茹、退茹分别只命中其中 12 种逐支顺行或逆行组合，进间、退间分别只命中其中 12 种每次隔一位顺行或逆行组合；四课克贼课体另穷举五种关系的 5⁴＝625 组轮廓；天网按 10 日干 × 12 占时 × 12 初传共 1,440 组穷举，上门乱首按 10 日干 × 12 日支 × 12 干上神 × 12 初传共 17,280 组穷举；九丑按 60 日柱 × 12 个大吉落地共 720 组穷举，六仪与旬三奇按 60 日柱 × 12 初传共 720 组穷举。九丑只取十个指定日柱且天盘大吉丑临本日支的严格条件，仪奇课体只取《六壬指南》明确的旬首或旬奇发用条件。上述规则只登记中性结构事实，不继承原典现实断语。完整排盘另验证 17,280 组月将、占时、日柱、昼夜组合，以及加入十二太岁后的 207,360 个课体上下文。
