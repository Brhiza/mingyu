...
- 重新审查紫微格局来源：原 84 条项目规则缺少逐条版本、卷页、原文和独立例盘，已整体退役；从零恢复 9 条可在《紫微斗数全书》卷一、卷三固定版本直接核对且具备可复算条件的规则，并向核心结果、提示词、公开 API 与 MCP 输出来源和边界。未登记格局不作判断。
...
- **七政四余传统盘暂停**：旧宿度链误把角宿起点固定为黄经 0°，古距度表合计 366.5 度且缺少距星边界校勘；命身宫、十二宫、庙旺与吊照停止输出近似结果，独立紫炁均速函数保留供研究。
- **西洋星盘固定真实出生瞬间**：真太阳时只作为传统时间参考证据，不再替换现代星历使用的民用出生时间；相位返回取消最强 12 条截断，保留阈值内全部结果。
- **统一出生档案保留传统时辰**：`BirthProfile` 支持直接提供 `timeIndex`，八字、紫微等时辰级算法不再被迫填写精确分钟；真太阳时和星盘仍严格要求精准时分与所需地点资料，七政四余完整传统盘当前暂停。
