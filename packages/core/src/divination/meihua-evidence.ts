...
export interface MeihuaSensoryOmenFact {
  ...
  promptText: string;
  sources: string[];
  limitation: '《万物赋》与《观物克应法》的外应必须来自耳闻目见的现场原始记录，并先区分成卦前后、观察时序、对象类别与实际状态，再与原卦及所占事项合参；当前时间戳、问题文本、主互变卦、设备麦克风或摄像头、历史回忆与事后挑选均不能替代，不得据缺失资料套用喧闹笑语、人物、鸟兽、器物、饮食、圆物、负土、刚健或柔腐等例断，也不得自动把观察标为已发生之事、未来之机、待辨物属性、吉凶或应期';
}
...
export interface MeihuaObjectContextFact {
  ...
  promptText: string;
  sources: string[];
  limitation: '《观物玄妙歌诀》《占物类例》《物数为体诀》《观物看变爻为主》《观物克应法》《观物趣时诀》《观物用易例》与《万物戏验》只适用于明确的观物、射覆或具体物件辨识，其中《万物戏验》更明确限定“猜手中物”；原文只可作为爻辞、八卦象、体用生克、用变互色数、多卦取体、阴阳爻多少、五六爻动、变后响应、现场与四时候选、两则历史实例及手中物目录的资料层级。当前输入没有专项情境和现场原始观察，底本又存在错题异文、句读冲突、统计范围、生旺与“有气”口径、卦象层级、规则合并及“六虚冲破”等未决问题。不得据规则目录、历史实例或普通盘面自动猜物，也不得生成主导卦、主导五行、物件种类、材质、形状、颜色、根部、气味、声响、软硬、湿烂焚毁、位置、完整缺损、价值、可用可食、成败结果、空手与否或精确数量等结论';
}
...
export interface MeihuaTenResponseContextFact {
  key: 'meihua:ten-response-context';
  status: '资料不足';
  ...
  promptText: string;
  sources: string[];
  limitation: '《占卜十应诀》只把正应、互应、变应回指当前已经结构化的主卦体用、体互与用互对原体、变卦用卦对原体事实，不重复计算或累计证据；方应、日应、刻应、外应、天时应、地理应与人事应必须另有现场方位、日应口径、耳闻目见、外物、天气、地点和人事原始记录。当前底本存在目录漏列方应、“此二卦之诀”、“火见雷为比和”、“并为体卦”、日应映卦与衰旺、诸应重叠及无限类推等未决问题，不得擅改原文或据此生成现场外应、日应吉凶、疾病诊断、痊愈、生死、评分、权重或概率';
}

export interface MeihuaMatterTenResponseContextFact {
  key: 'meihua:matter-ten-response-context';
  status: '资料不足';
  responseCatalogFields: string[];
  requiredContextFields: string[];
  availableContextFields: string[];
...
const SENSORY_OMEN_FACT_LIMITATION =
  '《万物赋》与《观物克应法》的外应必须来自耳闻目见的现场原始记录，并先区分成卦前后、观察时序、对象类别与实际状态，再与原卦及所占事项合参；当前时间戳、问题文本、主互变卦、设备麦克风或摄像头、历史回忆与事后挑选均不能替代，不得据缺失资料套用喧闹笑语、人物、鸟兽、器物、饮食、圆物、负土、刚健或柔腐等例断，也不得自动把观察标为已发生之事、未来之机、待辨物属性、吉凶或应期' as const;
...
const OBJECT_CONTEXT_FACT_LIMITATION =
  '《观物玄妙歌诀》《占物类例》《物数为体诀》《观物看变爻为主》《观物克应法》《观物趣时诀》《观物用易例》与《万物戏验》只适用于明确的观物、射覆或具体物件辨识，其中《万物戏验》更明确限定“猜手中物”；原文只可作为爻辞、八卦象、体用生克、用变互色数、多卦取体、阴阳爻多少、五六爻动、变后响应、现场与四时候选、两则历史实例及手中物目录的资料层级。当前输入没有专项情境和现场原始观察，底本又存在错题异文、句读冲突、统计范围、生旺与“有气”口径、卦象层级、规则合并及“六虚冲破”等未决问题。不得据规则目录、历史实例或普通盘面自动猜物，也不得生成主导卦、主导五行、物件种类、材质、形状、颜色、根部、气味、声响、软硬、湿烂焚毁、位置、完整缺损、价值、可用可食、成败结果、空手与否或精确数量等结论' as const;
...
const TEN_RESPONSE_CONTEXT_FACT_LIMITATION =
...
function buildSensoryOmenFact(): MeihuaSensoryOmenFact {
  ...
  return {
    ...
    missingObservationFields: [...requiredObservationFields],
    promptText:
      '当前输入未记录耳闻目见的现场原始事实、成卦前后时点、观察先后、对象类别与实际状态、当时明确关注对象及事项关联；不得把问题文字、时间戳、主互变卦、设备麦克风或摄像头、历史回忆或事后挑选补写成《万物赋》或《观物克应法》外应，也不能套用笑语哭泣、人物鸟兽、器物饮食、圆物、负土、刚健或柔腐等例断，或裁定已发生、未来、待辨物属性、吉凶与应期',
    sources: [
      '《梅花易数》卷三《万物赋》成卦前后、耳闻目见与原卦合参之法',
      '《梅花易数》卷三《观物克应法》第944至945行成卦未决时现场偶见物象候选',
...
function buildObjectContextFact(): MeihuaObjectContextFact {
  ...
  const bodySelectionRuleFields = [
    '物数之占另提出“不但以体卦为体”，不能把普通体用中的体卦视为唯一物体取主口径',
    '“凡卦之多者，皆可为体”只登记为重复卦象或五行的取体候选，统计对象、范围与并列时处理尚未确定',
    '乾金多只登记偏刚候选，坤土多只登记偏柔候选，不能据此直接确认材质或软硬',
    '体卦乾且互卦又见乾的例文列圆、健、刚、硬候选，但紧接“非金非石，此为体矣”句义未闭合',
    '体、互、变卦“并无生旺之气”只保留“不入五行物”原文候选，生旺判准与该词实际所指未明，不能据此分类',
  ];
  const lineStructureRuleFields = [
    '原文称阳爻多则多刚，只登记阴阳爻结构与偏刚候选，不直接确认物件刚硬',
    '原文称阴爻多则多柔，只登记阴阳爻结构与偏柔候选，不直接确认物件柔软',
    '动爻在五爻或上爻时只登记“多是能飞动之物”候选，“多是”不等于现实物件必能飞动',
  ];
  const changeObservationRuleFields = [
    '“凡观物”把本条限定在明确的观物、射覆或具体物件辨识，不是所有梅花问题的通用取主法',
    '正文“以变卦为主”只登记为变后响应候选，不直接覆盖普通体用主线，也不据此自动指定观物主导卦',
    '乾卦初爻动时，动爻所在下卦由乾变巽；原文只列“金刀削过之木物”候选，完整变卦为天风姤',
    '乾卦二爻动时，动爻所在下卦由乾变离；原文只列“火中锻炼之金”候选，完整变卦为天火同人',
    '乾卦三爻动时，动爻所在下卦由乾变兑；原文只列“毁折五金之器，虽圆而破处多”候选，完整变卦为天泽履',
  ];
  const responseOmenRuleFields = [
    '本条只适用于明确观物、射覆或具体物件辨识',
    '“又看体卦克应如何”只登记为体卦与现场克应需合参，正文没有给出确定生克计算式或优先顺序',
    '“成卦未决之际”要求当时现场原始观察及时间顺序，不能由事后回忆、问题文字、设备感知或盘面补造',
    '现场偶见圆物只登记圆形候选，不直接确认待辨物必为圆物',
    '现场见负土者只登记土中之物候选，不直接确认材质或埋藏状态',
    '现场见刚健或柔腐之物只登记相应属性候选，不直接确认待辨物的软硬、腐败或完整状态',
  ];
  const seasonalObservationRuleFields = [
    '“趣时察理”只登记为观物项下须合看四时与卦象的原则，不设为普通梅花问题的猜物入口',
    '春时得震、离只登记花类候选，不裁定震离二者任一或必须同见，也不直接确认为花',
    '夏时得震只登记有声之物候选，不因震卦或夏季单项条件确认物件有声',
    '秋时得兑只按原句登记“毁折成器之物”候选，在句义未闭合前不拆成损毁与成器两项确定属性',
    '冬时得坤只登记无用土物候选，不直接确认材质、用途或价值',
    '四句的“得”所指主卦、体卦、用卦、互卦、变卦或任意出现尚未确定，不用通用盘面自动匹配',
    '原文每季只举一组卦象，未覆盖的季节与未列卦象组合不得类推或补成完整物类表',
  ];
  const usageExampleFields = [
    '《观物用易例》第949至950行只保存两则历史命中案例，不构成八卦、爻辞、互卦、变卦与具体物件属性之间的通用规则表',
    '笼盛物例原文盘面为地天泰“初变升”、互见震兑；原断为草木类、生于土中、色青根黄、连根与“干根”，揭晓为新采于土中的连根草木',
    '笼盛物例可复算为地天泰初爻动成地风升，互卦为雷泽归妹、上震下兑；泰初九通行爻辞为“拔茅茹，以其汇。征吉。”，原文只节引前两句',
    '笼盛物例把节引爻辞、互震“清色”与兑“黄根”用于解释揭晓物，但“干根”“清色”及兑与黄根的对应均未闭合，只能保留原文解释与版本疑点',
    '锺覆物例原文盘面为火风鼎之雷风恒、互见乾兑；原断为有“声价气势”、虽圆而“面毁”、色白可用，揭晓为已经破损的“玉绦环”',
    '锺覆物例可复算为火风鼎上爻动成雷风恒，互卦为泽天夬、上兑下乾；鼎上九通行爻辞为“鼎玉铉，大吉，无不利。”，原文只节引“鼎玉铉，大吉”',
    '锺覆物例把节引爻辞、互乾兑与揭晓物合参，但“声价气势”“锺覆物”“虽圆面毁也”“玉绦环”的句义、观察泄漏与各层映射均未闭合，只能作为历史实例留档',
  ];
  const handGuessRuleFields = [
    '《万物戏验》开头明确作“凡猜手中物”，只登记手中藏物或握物专项规则目录，不是普通观物或所有梅花问题的通用八卦物象表',
    '乾只保存圆白、性刚、宝货及“有气为无价物”候选，不据乾卦自动确认形色、软硬、材质或价值',
    '坎只保存黑色、性柔、近水之物候选，不据坎卦自动确认颜色、软硬、湿度或位置',
    '艮只保存土中、瓦石、黄色、“有气”为成器及“逢兑克、无气”为折伤候选；兑出现层级与“有气”口径未定，不自动合并',
    '震、巽只保存竹木、有用、可食、当时果物、青色及刚柔候选；“遇兑之属可食”与“有气柔、无气刚”的句读和承接范围未定',
    '震、巽“遇坎”只保存污湿、或有气、无气烂朽之木候选；“遇”的卦象层级与条件关系未定',
    '离只原样保存“离色亦”、性柔、有水有木、火焚炭类及有气可货候选；“离色亦”疑有脱讹，未获可靠版本前不补字',
    '坤只保存土中、黄色、性温候选；兑只保存毁折与带口候选，不据单卦自动确认材质、温度、破损或开口',
    '春震巽、夏离、秋乾兑、冬坎只保存当令时可用或成器候选，“否则”为无用候选；季节分界、所得卦层级与未列组合均未说明',
    '“值六虚冲破，则必无物而空手”只保存原文空手候选；“六虚冲破”的术语、取值与计算条件未定义，不自动判定空手',
  ];
  ...
  const unresolvedRuleFields = [
    ...
    '第940行“凡卦之多者，皆可为体”没有说明统计主卦、互卦、变卦的哪些上下经卦，是按卦名还是五行归并，也没有并列最多时的处理规则',
    '第940行“非金非石，此为体矣”承接乾金圆健刚硬例而句义反折，现有同源转录未提供可靠句读或异文互证',
    '第940行“体互变卦，并无生旺之气者，为不入五行物”没有说明生旺所据时令与判准，“不入五行物”的实际物类含义也未闭合',
    '第940行“八卦中阳爻多”没有明确是统计本卦六爻还是体、互、变所含爻象，也没有阴阳相等时的处理规则',
    '第938行以常规体卦为主，第940行又称多卦可为体，第943行再称以变卦为主，原文没有说明三种取主口径各自的适用范围与优先顺序',
    '第942行标题作“观物看变爻为主”，第943行正文却作“以变卦为主”，“应用之应验也”句读与语义也未闭合；仅见现代编辑本改作“以变爻为主”“木器”，但没有可靠版本依据',
    '第943行三例称初、二、三爻分别“变为”巽、离、兑，而完整六爻变卦实际为姤、同人、履，不能确定一般规则中的“变卦”究竟指完整六爻变卦还是动爻所在经卦的变后结果',
    '第943行只列纯乾卦初至三爻三个例子，未覆盖乾卦四爻至上爻及其他六十三卦，不能推广成全盘猜物算法',
    '第945行“物之成败”没有说明是猜物判断是否命中、物件成器或毁败，还是现实事项成败，不能泛化',
    '第945行“体卦克应如何”没有说明所用克应字段、体卦生克关系、与四个现场例子的合并或优先顺序；四例本身也没有实际使用体卦',
    '第945行“有见……相遇/过”没有说明取第一所见、全部所见还是最显著对象，多项冲突时如何处理也未交代；2025年现代编辑本删去负土例并误并刚健句，没有可靠版本依据',
    '第947行“趣时察理”没有说明是随四时取象、按旺衰修正，还是另有时令规则',
    '第947行四次“得”没有指明所得为主卦、体卦、用卦、互卦、变卦或盘内任意出现的经卦',
    '第947行“春得震、离为花”没有说明震与离为任一即成还是必须组合同见',
    '第947行夏震为有声之物与震雷声类象的关系只是可能解释，原文没有提供判定条件或强度口径',
    '第947行“毁折成器之物”没有可靠句读，不能确定是毁折、成器两类并列，还是一项复合状态',
    '第947行冬坤为无用土物没有说明是否仍须合看旺衰、体用生克或其他卦象',
    '第947行的春夏秋冬没有说明按节气、农历月、月建地支或其他口径分界；当前项目按节气划季、缺失时退回农历月，不能反证原文口径',
    '第947行每季只列震离、震、兑或坤一组例证，没有覆盖同季其他卦象，不能补成八卦乘四时的完整规则表',
    '第949至950行两例均没有记录起卦时间、数字、取数算式或现场起卦方法，只能复算既述卦画，不能复现为何先得到泰或鼎',
    '第949行“地天泰之初变升”可由泰初爻动复算为升，但“初”未写作“初爻”，不能据同源转录擅自改写底本',
    '第949行“干根”句义不明，且与揭晓所述“新采于土中”的连根草木可能冲突，没有可靠异文足以改字或确定物性',
    '第949行前称“色青”、后称“互震为清色”，“清色”可能是“青色”讹字，但现有公开转录基本同源，不能据现代改写覆盖底本',
    '第949行“兑为黄根”与通行兑卦白色类象不一致，也没有说明由兑推到黄色根部的中间依据，不能扩展为兑卦通则',
    '第950行没有明写上爻动，只能由鼎变恒的唯一卦画变化及所引鼎上九爻辞反推；这种复算不能补成原文未载的起卦过程',
    '第950行“声价气势”句义不闭合，现代编辑本改作“身价气势”但没有独立版本依据，不能据此生成声响、价值或气势属性',
    '第950行仅称“以锺覆物”，没有说明覆盖物的材质、形状或声响是否在判断时可见可闻，不能排除由外部容器泄漏答案属性',
    '第950行前称“虽圆而今毁缺矣”、后称“虽圆面毁也”，“面”字的归属与句读不明，现代改作“而毁”没有可靠版本依据',
    '第950行“玉绦环”的具体器物身份不明，也没有逐项说明玉、环、白、圆、可用、破损分别来自爻辞、本卦、互卦还是变卦',
    '第949至950行只列两个揭晓命中的成功案例，没有失败样本，也未覆盖其他卦、爻、物类与遮覆方式，不能推广到盘内相同卦象或其他观物问题',
    '两例没有交代爻辞、本卦、互卦、变卦、体用、时令与现场覆盖物之间的合并及优先顺序，不能建立自动匹配历史实例或输出预测物件的算法',
    '第952行开头明确限定“猜手中物”，没有说明如何起卦、手中物是否可见、由谁持有及与前文一般观物规则如何衔接，不能扩展为普通问题的通用物象表',
    '第952行反复使用“有气”“无气”，但没有定义是月令旺衰、卦气、生克得助、物件生机还是其他口径，不能直接等同项目现有四时旺衰',
    '第952行乾“有气为无价物”中的“无价”可表示极贵或不可估价，亦未说明与“宝货”的关系，不能自动生成价值判断',
    '第952行艮“逢兑克，无气，折伤之物”没有说明兑位于主、体、用、互、变何层，“无气”是并列还是受克后的条件，不能自动判定破损',
    '第952行“遇兑之属可食，当时之果物”承接震巽、兑或另起一句不明，“之属”与可食、果物、青色之间的适用范围也未闭合',
    '第952行“有气柔，无气刚”紧接果物段，但承接范围不明，没有说明是指震巽、遇兑、当时果物还是全段手中物，不能据旺衰推断软硬',
    '第952行“震、巽遇坎”中的“遇”没有说明是主互变组合、上下经卦同见、体用关系还是其他层级，“或有气”与前后条件的句读也不明',
    '第952行“离色亦”句义残缺，可能存在脱字或讹字；现有公开转录均保留同文且明显同源，没有可靠独立版本足以改作“赤”等字',
    '第952行离段“有水有木，而火焚之”没有说明水、木、火分别来自哪些卦层及是否须同时出现，“有气”为可货之物又与前句如何衔接不明',
    '第952行春震巽、夏离、秋乾兑、冬坎只列部分卦象，没有说明“得”的层级、季节分界、同季未列卦象，以及“否则”是指非当令卦还是所有未列组合',
    '第952行“六虚冲破”没有定义六虚所指、冲破对象、所据干支或卦爻条件，不能直接映射六爻空破、旬空、六冲或项目现有任一计算字段',
    '第952行没有交代八卦目录、“有气”、卦象相遇、四时、六虚冲破与前文爻辞、体用、互变、现场克应之间的合并及优先顺序，不能建立自动猜物算法',
  ];
  return {
    key: 'meihua:object-context',
    status: '资料不足',
    ...
    relationRuleFields,
    quantityRuleFields,
    bodySelectionRuleFields,
    lineStructureRuleFields,
    changeObservationRuleFields,
    responseOmenRuleFields,
    seasonalObservationRuleFields,
    usageExampleFields,
    handGuessRuleFields,
    sourceLineFields,
    unresolvedRuleFields,
    promptText:
      '当前输入未明确观物专项所需情境：现只保留起卦时点与取数、主互变上下经卦、动爻与普通体用、月份与四时旺衰等盘面事实；尚未确认所占确为观物或射覆、是否为“猜手中物”、待辨物件范围与可见遮覆手持状态、所需辨识的属性范围，也未记录成卦未决时的现场原始观察及先后次序。《占物类例》第935至938行、《物数为体诀》第939至941行、《观物看变爻为主》第942至943行、《观物克应法》第944至945行、《观物趣时诀》第946至947行、《观物用易例》第948至950行及《万物戏验》第951至952行已留档，只确认相关规则目录、现场与四时候选、两则历史实例及手中物目录的资料层级；“有气”、卦象相遇、“离色亦”、季节“否则”及“六虚冲破”等条件仍未闭合。《万物戏验》只作历史规则目录，不得自动猜手中物，不得把规则目录或成功实例套到普通问题，也不得输出预测或揭晓物件、主导卦、主导五行、具体物件、材质、形色、根部、声响、软硬、湿烂焚毁、破损开口、价值、可用可食、空手与否、成败结果或精确数量',
    sources: [
      '《梅花易数》卷三《观物玄妙歌诀》物形、物色、物性、位置、体质、卦象组合与四时取象',
      '《梅花易数》卷三《占物类例》第935至938行爻辞、八卦象、体用生克与互卦数例',
      '《梅花易数》卷三《物数为体诀》第939至941行多卦取体、阴阳爻多少与五六爻动候选',
      '《梅花易数》卷三《观物看变爻为主》第942至943行及乾卦前三爻变后经卦与完整变卦复算',
      '《梅花易数》卷三《观物克应法》第944至945行体卦克应与成卦未决时现场物象候选',
      '《梅花易数》卷三《观物趣时诀》第946至947行四时与震离兑坤物象候选',
      '《梅花易数》卷三《观物用易例》第948至950行泰变升、鼎变恒两则历史实例及揭晓结果',
      '《梅花易数》卷三《万物戏验》第951至952行手中物八卦、卦象相遇、四时与“六虚冲破”候选目录',
      '通行《周易》泰初九、鼎上九爻辞与两例主卦、互卦、变卦卦画复算',
      '通行《周易》乾、坤、困卦爻辞与当前底本逐句对照',
      '多个公开转录与现代编辑异文交叉对照；公开文本多属同源转载，不能替代独立版本校勘',
...
function buildTenResponseContextFact(
  data: MeihuaData,
  stages: MeihuaStageEvidence[],
  interResponseFacts: MeihuaInterResponseEvidence[],
): MeihuaTenResponseContextFact {
  const responseCatalogFields = [
    '正应',
    '互应',
    '变应',
    '方应',
  ...
  const resultStage = stages.find((item) => item.stage === 'result');
  const tiInter = interResponseFacts.find((item) => item.role === '体互');
  const yongInter = interResponseFacts.find((item) => item.role === '用互');
  const reusedInternalResponseFields = [
    `正应：只回指${originStage?.key ?? '缺失的主卦体用事实'}，不重复计算或累计证据`,
    `互应：只回指${tiInter?.key ?? '缺失的体互事实'}与${yongInter?.key ?? '缺失的用互事实'}，分别响应原体，不把互卦内部重分体用`,
    `变应：只回指${resultStage?.key ?? '缺失的变卦体用事实'}中变卦用卦对原体的关系，不重复计算或累计证据`,
  ];
  const requiredContextFields = [
    '方应所需的占者位置、来占者相对占者的实际方位及坐立状态',
    '方应观察中心与既有坐端八方应兆之间的明确区分口径',
    '日应所占事项“属何卦”的取卦方法与同五行两经卦的选择规则',
    '本日卦气与体卦衰旺的独立算法、采用版本及与月令旺衰的合参规则',
    '刻应所需的占卜当刻耳闻目见原始记录、准确时点与观察先后',
    '外应所需的偶见外物原始记录、实际属性、来去动静与映卦依据',
    '天时应与地理应所需的现场实际天气、地点和环境特征',
    '人事应所需的现场实际人物、事件、言语行为、时序与映卦依据',
  ];
  const sourceLineFields = [
    '断占总诀篇之四',
    '占卜十应诀',
  ...
  const unresolvedRuleFields = [
    '第956行总表自称“十应”却只列九项，漏列后文独立成节的方应；公开转录多属同源电子底本，现按正文十节留档但不擅改总表',
    '正应、互应、变应共列三项后原文写“此二卦之诀”，疑有脱讹；未获独立版本互证前不改字、不据此删并任何一项',
    '方应所称“来占之人在何方位上”可能以来占者相对占者的位置为准，与既有“求测者所在处为中心”的坐端应兆不是同一观察口径',
    '日支只映射木火金水土，未说明同五行的两经卦如何选择；“所占属何卦”“本日衰旺”的算法及其与现有月令旺衰的关系均未闭合',
    '刻应、外应与人事应都可能使用占卜当刻耳闻目见的人事物象，原文没有给出重复观察去重、优先级、多项并见或相互冲突的处理规则',
    '“火见雷为比和”与现有五行中震木生离火的关系冲突，且同篇前文亦有离体得震巽为吉的木生火方向；不得用该句覆盖公共五行关系',
    '地理应末句“并为体卦”与全篇以体卦为主、外应合参的结构可能冲突；公开同源转录均保留原字，未作独立版本互证前不得改成“用卦”或直接执行',
    '末句“其余占卜，并以类推之”没有给出有限对象、映射表、停止条件与冲突处理，不能据此无限扩展新规则',
  ];
  const highRiskRuleFields = [
    '疾病段只保存历史原文，不得把互变克体、本卦无生体或体卦旺衰用于现实疾病诊断',
    '“庶几有望”“无复生理”“不可望安”等文字不得输出为痊愈、恶化或生死结论',
    '外应、人事应的哭笑、人物与事件不得反向补造成患者病因、事故原因或现实风险事实',
    '十应项目、五行关系或现场观察不得换算吉凶评分、权重、概率或确定应期',
  ];
  return {
    key: 'meihua:ten-response-context',
    status: '资料不足',
    responseCatalogFields,
    reusedInternalResponseFields,
    requiredContextFields,
    availableContextFields: [],
    missingContextFields: [...requiredContextFields],
    availableChartFields: [
      `主卦体用事实：${originStage?.key ?? '缺失'}`,
      `互卦响应事实：${tiInter?.key ?? '缺失'}、${yongInter?.key ?? '缺失'}`,
      `变卦体用事实：${resultStage?.key ?? '缺失'}`,
      `起卦日干支${data.ganzhi.day}及日支${data.ganzhi.day.slice(-1)}，只作原始日期资料`,
      `月支${data.ganzhi.month.slice(-1)}与既有四时旺衰，只作月令资料，不能替代“本日衰旺”`,
    ],
    sourceLineFields,
    unresolvedRuleFields,
    highRiskRuleFields,
    promptText:
      '《占卜十应诀》第954至978行及正、互、变、方、日、刻、外、天时、地理、人事十项目录已完整留档。正应、互应、变应三项只回指现有主卦体用、体互与用互分别响应原体、变卦用卦响应原体的结构化事实，不重复计算或累计证据；当前没有方应、刻应、外应、天时应、地理应、人事应所需的现场原始观察，也没有日应的取卦、本日卦气与衰旺闭合算法。日支虽已记录，但不能自动生成日应吉凶。底本的目录漏项、“此二卦之诀”、“火见雷为比和”、“并为体卦”、诸应重叠与无限类推均保持未决，不得套用为现场外应、疾病诊断、痊愈、生死、吉凶评分、权重、概率或确定应期',
    sources: [
      '《梅花易数》卷三《断占总诀篇之四·占卜十应诀》第954至978行',
      '《梅花易数》卷三《十应奥论》《十应目论》及复明诸应第694至719行交叉核对',
      '多个公开转录的目录漏项与关键异文对照；公开文本多属同源电子底本，不能替代独立版本校勘',
      '当前梅花主互变结构化事实、日干支、月令与现场情境输入逐项对照',
    ],
    limitation: TEN_RESPONSE_CONTEXT_FACT_LIMITATION,
  };
}

function buildMatterTenResponseContextFact(data: MeihuaData): MeihuaMatterTenResponseContextFact {
  const responseCatalogFields = ['行', '立', '坐', '卧', '担', '券', '裹头', '跣足', '喜', '怒'];
  const requiredContextFields = [
    '现场实际观察到的行为、姿态、装束或情绪原始记录',
    '被观察者身份及其与求测者的关系',
    '观察发生在起卦前、起卦中还是起卦后',
    '准确观察时间、多项观察先后与持续情况',
    '官事、财、信、客、文书、疾病等明确事项、具体对象与判断目标',
    '“属木、属火、金水土”所指日干、日支、纳音、卦象或其他对象的版本口径',
    '“旺木”等日辰旺衰算法及其与现有月令旺衰的关系',
    '多项观察并见、冲突、重复时的去重、优先级与适用规则',
  ];
  const sourceLineFields = [
    '论事十大应（论日辰秘文）',
  ...
  const unresolvedRuleFields = [
    '本节与前一节《占卜十应诀》的关系、适用先后及是否同属现场克应未说明，不能合并为同一套十应',
    '标题“论日辰秘文”未说明具体使用日干、日支、纳音、干支五行、日卦或其他日辰层级',
    '担、券、裹头、跣足等十项究竟指现场动作、姿态、装束、情绪还是其他分类，底本未给定义',
    '各行句读、主语、事项分段以及木火金水土与前后短句的绑定范围不明',
    '官事、官司、口舌等名目是否属于同一事项，以及各自适用条件不明',
    '财、信、客、文书、疾病等分句的起止与适用范围不明，不能从词序重组算法',
    '“病却月”“犯林木神”“阴人小口灾”“病有孝至”等句义及版本均未闭合',
    '多项动作、姿态、装束或情绪并见时，没有去重、冲突、重复或优先级规则',
    '现代转录所见“行、立、坐、卧、语、默、喜、怒、得、失”目录缺少可靠版本来源，不能覆盖当前底本',
    '多个公开转录明显沿用同一电子底本，不能据其一致性证明担、券等底本文字无误',
  ];
  const highRiskRuleFields = [
    '不得生成官司是否发生、胜负、是否见官、受欺凌或破财等法律与财务结果',
    '不得生成贵人、客人到来、文书发动、口舌或他人行为等现实事件',
    '不得生成潮热、四肢沉重、病凶、痊愈或其他疾病诊断、治疗与预后结论',
    '不得把“犯林木神”“阴人小口灾”“病有孝至”等解释为鬼神、性别年龄或丧事因果',
    '不得从现有日干支、月令旺衰、主互变卦或问题关键词自动选择十应、事项或五行条件',
    '不得把目录、原文或缺失资料换算为吉凶、评分、权重、概率或确定应期',
  ];
  return {
    key: 'meihua:matter-ten-response-context',
    status: '资料不足',
    ...
    missingContextFields: [...requiredContextFields],
    availableChartFields: [
      `起卦日干支${data.ganzhi.day}及日支${data.ganzhi.day.slice(-1)}，只作原始日期资料，不能据此确定本节五行口径`,
      `月支${data.ganzhi.month.slice(-1)}及现有四时旺衰，只作月令资料，不能替代标题所称日辰旺衰`,
      `主卦${data.mainHexagram.name}、互卦${data.interHexagram?.name ?? '缺失'}、变卦${data.changedHexagram?.name ?? '缺失'}，只作盘面资料，不能补造现场十应`,
    ],
    sourceLineFields,
    unresolvedRuleFields,
    highRiskRuleFields,
    promptText:
      '《论事十大应（论日辰秘文）》第979至989行及行、立、坐、卧、担、券、裹头、跣足、喜、怒十项目录已完整留档。当前没有现场行为、姿态、装束或情绪原始记录、被观察者身份、观察时序和明确专项事项；底本也没有闭合“属木、属火、金水土”的所指、“旺木”等日辰旺衰、句读绑定及多项并见规则。现有日干支、月令旺衰、主互变卦、问题关键词与前节占卜十应均不能替代这些资料，不生成现场应兆、法律或财务结果、贵人访客、文书发动、疾病诊断或预后、鬼神因果、吉凶、评分、权重、概率或确定应期',
    sources: [
      '《梅花易数》卷三《论事十大应（论日辰秘文）》第979至989行',
      '多个公开转录对担、券、裹头、跣足等字样的交叉核对；公开文本明显同源，不能替代独立版本校勘',
      '现代“行立坐卧语默喜怒得失”目录与当前底本对照；因无可靠版本来源，不据其校改原文',
      '当前梅花输入、日干支、月令旺衰、主互变盘面与现场专项资料逐项对照',
    ],
    limitation: MATTER_TEN_RESPONSE_CONTEXT_FACT_LIMITATION,
  };
}
  ...
  return [
    ...
    },
    {
      ...
      result: {
        ...
        tenResponseContextStatus: params.tenResponseContextFact.status,
        tenResponseCatalogCount: params.tenResponseContextFact.responseCatalogFields.length,
        reusedInternalResponseCount:
          params.tenResponseContextFact.reusedInternalResponseFields.length,
...
function buildLimitationFacts(params: {
  ...
  > = [
    ...
    },
    {
      ...
      promptText:
        '《万物赋》与《观物克应法》取应先须保留耳闻目见的现场原始事实，区分成卦前后与观察先后，再核对对象类别、实际状态、当时关注对象及所占事项；当前七项资料均未输入时，不得从问题文字、时间戳、卦象、设备感知、历史回忆或事后挑选补造外应，也不得直接套用笑语哭泣、人物鸟兽、器物饮食、圆物、负土、刚健或柔腐等例断，或标定已发生、未来与待辨物属性',
      sources: ['《梅花易数》卷三《万物赋》《观物克应法》与当前耳目现场资料覆盖'],
    },
    {
    ...
    },
    {
      ...
      promptText:
        '《观物玄妙歌诀》《占物类例》《物数为体诀》《观物看变爻为主》《观物克应法》《观物趣时诀》《观物用易例》与《万物戏验》须先确认所占确为观物、射覆或具体物件辨识；《万物戏验》还须明确确为“猜手中物”，并提供待辨对象范围、可见遮覆手持状态、所需属性范围及现场原始观察。当前资料均未输入，底本与规则仍有四十八项缺口。手中物目录与两则历史实例均不构成通用算法，不得自动匹配或猜物，也不得套出物件种类、材质、形色、根部、声响、软硬、湿烂焚毁、破损开口、价值、用途、可食性、空手与否、成败结果或精确数量',
      sources: [
        '《梅花易数》卷三《观物玄妙歌诀》《占物类例》《物数为体诀》《观物看变爻为主》《观物克应法》《观物趣时诀》《观物用易例》《万物戏验》、通行《周易》爻辞与卦画对照及当前观物专项资料覆盖',
      ],
    },
    {
    ...
    {
      key: 'meihua:limitation:ten-response-context',
      type: '占卜十应边界',
      ownerFactKeys: [params.tenResponseContextFact.key],
      promptText:
        '《占卜十应诀》十项目录只完整留档原文层级：正、互、变三应回指既有盘面事实，不重复计证；方、日、刻、外、天时、地理、人事七应因现场与日应算法资料不足而不计算。目录漏列方应、“此二卦之诀”、“火见雷为比和”、“并为体卦”及诸应重叠等问题未完成独立版本互证，不擅改、不类推；疾病末段不得生成诊断、痊愈或生死结论',
      sources: [
        '《梅花易数》卷三《占卜十应诀》第954至978行、前文《十应奥论》《十应目论》与当前资料覆盖',
      ],
    },
    {
      key: 'meihua:limitation:matter-ten-response-context',
      type: '论事十大应边界',
      ownerFactKeys: [params.matterTenResponseContextFact.key],
      promptText:
        '《论事十大应》仅完整留档行、立、坐、卧、担、券、裹头、跣足、喜、怒及第979至989行原文。当前没有现场观察、被观察者身份、观察时序和明确专项事项，底本也没有闭合五行所指、日辰旺衰、句读绑定及多项并见规则；不得从日干支、月令、卦象、问题关键词或前节占卜十应补造应兆，更不得生成法律财务、贵人访客、文书、疾病、鬼神因果或量化结论',
      sources: [
...
export function analyzeMeihuaEvidence(data: MeihuaData): MeihuaEvidenceAnalysis {
  ...
  const items: PromptEvidenceItem[] = [
    ...
    {
      level: '限制',
      title: '观物专项、占物类例、物数为体、变爻、现场克应、趣时与用易实例版本覆盖',
    ...
    {
      level: '限制',
      title: '占卜十应目录、复用关系与资料边界',
      detail: `${tenResponseContextFact.promptText}；边界：${tenResponseContextFact.limitation}`,
      source: tenResponseContextFact.sources.join('、'),
      tags: [
        '占卜十应诀',
        '正互变三应复用',
        '七应资料不足',
        '版本互证',
        '疾病高风险边界',
        tenResponseContextFact.status,
      ],
    },
    {
      level: '限制',
      title: '论事十大应目录、现场资料与日辰口径边界',
      detail: `${matterTenResponseContextFact.promptText}；边界：${matterTenResponseContextFact.limitation}`,
      source: matterTenResponseContextFact.sources.join('、'),
      tags: [
        '论事十大应',
        '论日辰秘文',
        '现场观察资料不足',
  ...
  return {
    ...
    methodology: [
      ...
      '《万物赋》与《观物克应法》耳目外应须保留现场原始事实，区分成卦前后、观察先后、对象类别与实际状态，再与原卦及所占事项合参；资料缺失时不套用笑语哭泣、人物鸟兽、器物饮食、圆物、负土、刚健或柔腐等例断。',
      '《饮食篇》只用于明确的具体饮食、宴请、食物或能否得食之占；须另行明确判断对象、己人客酒食物专项角色、动静所指与原始记录、宴食时间及主客关系，且在艮、坎段落与末段句读完成版本校勘前不生成具体饮食或宾主结论。',
      '《观物玄妙歌诀》《占物类例》《物数为体诀》《观物看变爻为主》《观物克应法》《观物趣时诀》《观物用易例》与《万物戏验》只用于明确的观物、射覆或具体物件辨识；《万物戏验》更须明确确为“猜手中物”，并另行提供待辨对象范围、可见遮覆手持状态、所需属性范围及现场原始观察。这里只登记规则目录、现场与四时候选、两则历史实例及手中物目录的资料层级；“有气”、卦象相遇、“离色亦”、季节“否则”及“六虚冲破”等条件未闭合前，不自动匹配历史实例或猜物，也不生成物件种类、形色材质、根部、声响、软硬、湿烂焚毁、破损开口、价值、用途、可食性、空手与否、成败结果或精确数量。',
