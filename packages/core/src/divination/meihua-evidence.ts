...
export interface MeihuaObjectContextFact {
  ...
  promptText: string;
  sources: string[];
  limitation: '《观物玄妙歌诀》《占物类例》与《物数为体诀》只适用于明确的观物、射覆或具体物件辨识；前两篇可确认爻辞、八卦象、体用生克与用变互色数层级，后一篇另列多卦取体、阴阳爻多少及五六爻动等候选规则。但当前输入没有专项情境，底本又存在艮离错题、爻辞异文、句读冲突、统计范围、生旺口径、不入五行物句义及后续变卦取主范围等未决问题。不得据规则目录或普通盘面生成物件种类、材质、形状、颜色、气味、软硬、动静、位置、完整缺损、价值、可用可食或精确数量等结论';
}
...
const OBJECT_CONTEXT_FACT_LIMITATION =
  '《观物玄妙歌诀》《占物类例》与《物数为体诀》只适用于明确的观物、射覆或具体物件辨识；前两篇可确认爻辞、八卦象、体用生克与用变互色数层级，后一篇另列多卦取体、阴阳爻多少及五六爻动等候选规则。但当前输入没有专项情境，底本又存在艮离错题、爻辞异文、句读冲突、统计范围、生旺口径、不入五行物句义及后续变卦取主范围等未决问题。不得据规则目录或普通盘面生成物件种类、材质、形状、颜色、气味、软硬、动静、位置、完整缺损、价值、可用可食或精确数量等结论' as const;
...
function buildObjectContextFact(): MeihuaObjectContextFact {
  ...
  const unresolvedRuleFields = [
    ...
    '第938行以常规体卦为主，第940行又称多卦可为体，第943行《观物看变爻为主》再以变卦为主，三者适用范围与优先顺序尚待后文合校',
  ];
  return {
    key: 'meihua:object-context',
    status: '资料不足',
    ...
    promptText:
      '当前输入未明确观物专项所需情境：现只保留起卦时点与取数、主互变上下经卦、动爻与普通体用、月份与四时旺衰等盘面事实；尚未确认所占确为观物或射覆、待辨物件范围与可见遮覆状态、所需辨识的属性范围。《占物类例》第935至938行与《物数为体诀》第939至941行已留档，可确认爻辞、八卦象、体用生克、用变互色数，以及多卦取体、阴阳爻多少和五六爻动等传统资料层级；但艮离错题、爻辞异文、句读冲突、卦数统计范围、生旺判准、不入五行物句义及后续变卦取主规则仍未闭合。不得把规则目录套到普通问题，也不得输出具体物件、材质、形色、状态、价值、可用可食或精确数量',
    sources: [
...
function buildLimitationFacts(params: {
  ...
  > = [
    ...
    },
    {
      ...
      promptText:
        '《观物玄妙歌诀》《占物类例》与《物数为体诀》须先确认所占确为观物、射覆或具体物件辨识，并明确待辨对象范围、可见遮覆状态和所需属性范围；当前三项情境均未输入。原文虽列爻辞、八卦象、体用生克、用变互色数、多卦取体、阴阳爻多少与五六爻动等资料层级，但底本仍有艮离错题、爻辞异文、句读冲突、统计范围、生旺判准、不入五行物句义与后续取主规则十类缺口。不得以问题关键词、规则目录或通用盘面套出具体物件及其材质、形色、状态、价值、用途、可食性或精确数量',
      sources: [
        '《梅花易数》卷三《观物玄妙歌诀》《占物类例》《物数为体诀》、通行《周易》爻辞对照与当前观物专项资料覆盖',
      ],
    },
    {
...
export function analyzeMeihuaEvidence(data: MeihuaData): MeihuaEvidenceAnalysis {
  ...
  return {
    ...
    methodology: [
      ...
      '《观物玄妙歌诀》《占物类例》与《物数为体诀》只用于明确的观物、射覆或具体物件辨识；须另行明确待辨对象范围、可见遮覆状态和所需属性范围。这里只登记爻辞、八卦象、体用生克、用变互色数、多卦取体、阴阳爻与五六爻动等传统规则目录；艮离错题、爻辞异文、句读冲突、数量与卦象统计范围、生旺判准、不入五行物句义和后续取主范围未闭合前，不生成具体物件、形色材质、状态、价值、用途、可食性或精确数量。',
