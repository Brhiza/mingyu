        ...
        item.name === '伏干格' || item.name === '飞干格'
          ...
          : item.name === '岁格'
            ...
            : item.name === '格勃'
              ...
              : item.name === '天乙飞宫格' || item.name === '天乙伏宫格'
                ? [
                    '《奇门遁甲统宗》“庚加值符天乙伏，值符加庚天乙飞”',
                    '《奇门旨归》“六甲值符加地庚为飞宫格”“六庚加地甲值符为伏宫格”',
                    '《奇门法窍》天乙飞宫、天乙伏宫核心结构',
                    '《遁甲演义》“六庚加值符为伏宫”“值符加六庚为飞宫”',
                    '当前完整时柱、旬首所遁六仪、值符实际携干与宫内天地盘干复算记录',
                  ]
                : item.name === '相佐'
                  ? [
                      '《遁甲演义》“符加丙丁为相佐”',
                      '《奇门遁甲秘笈大全》“符加丙兮为相佐”',
                      '《奇门旨归》“本旬值符加地盘三奇之上”为相佐',
                      '《奇门法窍》“旧法有直符加三奇，谓之相佐”',
                      '当前完整时柱、旬首所遁六仪、值符实际携干与地盘丙复算记录',
                    ]
...
// 旧 stemRelations 不是上下文格的可信来源；即使岁格、格勃、天乙飞伏宫已开放，也必须由完整
...
export function analyzeQimenEvidence(input: QimenData): QimenEvidenceAnalysis {
  ...
  const ruleSourceFacts: QimenRuleSourceFact[] = [
    ...
    {
      key: 'rule:qimen:tian-yi-fei-fu-gong',
      status: '已声明',
      category: '天乙飞伏宫时家结构规则',
      rule: '天乙飞宫格仅在时家当前值符旬首所遁六仪临地盘庚时命中；天乙伏宫格仅在天盘庚临地盘值符旬首所遁六仪时命中；甲申旬值符与六庚身份重合，自动命名失败关闭',
      appliesTo: ['天乙飞宫格', '天乙伏宫格', '完整时柱', '值符实际携干', '六庚落宫'],
      sources: [
        '《奇门遁甲统宗》“庚加值符天乙伏，值符加庚天乙飞”',
        '《奇门旨归》“六甲值符加地庚为飞宫格”“六庚加地甲值符为伏宫格”',
        '《奇门法窍》天乙飞宫、天乙伏宫核心结构',
        '《遁甲演义》“六庚加值符为伏宫”“值符加六庚为飞宫”',
        '当前完整时柱、旬首所遁六仪、值符唯一落宫及天地盘干复算记录',
      ],
      promptText:
        '天乙飞伏宫时家结构：只有排盘明确列出的天乙飞宫格、天乙伏宫格可作为中性结构事实引用。飞宫要求值符星实际携当前旬首所遁六仪并临地盘庚；伏宫要求天盘庚临地盘该值符旬首所遁六仪。甲申旬中值符本身即六庚，两种身份无法区分，均不得自动命名。不得据格名生成兵事胜负、主客判断、贵人状态、吉凶、行动或方位建议；月家、年家不得套用',
      limitation: RULE_SOURCE_LIMITATION,
    },
    {
      key: 'rule:qimen:xiang-zuo-shou-hu-boundary',
      status: '已声明',
      category: '相佐与守户审核边界',
      rule: '相佐仅开放四书共同交集“时家当前值符临地盘丙”的中性结构；地盘乙、丁不扩大命名。守户因值使临地丁、时干临地丁及守星/守门/守户异名冲突，继续失败关闭',
      appliesTo: ['相佐', '守户', '完整时柱', '值符实际携干', '值使门', '地盘丙丁'],
      sources: [
        '《遁甲演义》“符加丙丁为相佐，使加六丁为守户”',
        '《奇门遁甲秘笈大全》“符加丙兮为相佐，时加六丁为守星”',
        '《奇门旨归》“本旬值符加地盘三奇之上”为相佐',
        '《奇门法窍》“旧法有直符加三奇，谓之相佐”及“直使加地丁为守门”',
        '当前完整时柱、值符唯一落宫、值符实际携干、值使门落宫与宫内天地盘干复算记录',
      ],
      promptText:
        '相佐与守户审核边界：只有排盘明确列出的相佐可作为中性结构事实引用，且必须同时满足时家完整时柱、值符星唯一落宫并实际携当前旬首所遁六仪、该宫地盘干为丙。地盘乙、丁虽见于部分相佐版本，不自动扩大命名。守户在《遁甲演义》的“使加六丁”、秘笈的“时加六丁为守星”及《奇门法窍》的“直使加地丁为守门”之间名称和条件未统一，不得自动命名；现有六个固定时柱且值使临地丁的玉女守门仍按独立已审核规则处理。不得据此生成助力、贵人、门户安全、兵事、行动、方位或吉凶结论；月家、年家不得套用',
      limitation: RULE_SOURCE_LIMITATION,
    },
    {
      key: 'rule:qimen:fu-shi-same-palace-name-boundary',
      status: '已声明',
      category: '符使同宫名称边界',
      rule: '《奇门宝鉴御定》的“符使同宫”说明值符和值使同起于旬首甲遁之宫、随后分别加临，不足以把当前值符星和值使门同落一宫命名为独立吉格；自动格名失败关闭，同宫时仍分别保留值符、值使落宫事实',
      appliesTo: ['值符落宫', '值使落宫', '同宫位置事实', '符使同宫旧标签'],
      sources: [
        '《奇门宝鉴御定》“符使同起于甲遁之宫，各异于加临之所也”',
        '《奇门宝鉴御定》“星符门使同宫起，易一时兮即分矣。符从甲去寻时干，使索时支遁处止”',
        '《奇门法窍》六甲时古例所列“星门符伏”“星符门（伏）”实际位置记录',
        '当前值符星唯一落宫、值使门唯一落宫与九宫位置索引复算记录',
      ],
      promptText:
        '符使同宫名称边界：《奇门宝鉴御定》所述是值符、值使同起于旬首甲遁之宫并随时分别加临的排布机制，不是“当前值符星和值使门同落一宫即为独立吉格”的可靠定义。两者当前同宫时，只能分别引用“值符落宫”和“值使落宫”两个可复算位置事实，不得自动命名符使同宫，也不得附加现实成败、力量判断、结果概率或行动断语',
      limitation: RULE_SOURCE_LIMITATION,
    },
    {
    ...
    {
      key: 'rule:qimen:star-door-fuyin-fanyin-hour-position',
      status: '已声明',
      category: '星门伏吟反吟时家位置规则',
      rule: '星门伏吟反吟只按时家实际位置判断：值符星或值使门回各自本位为伏吟，落本位对冲宫为反吟；不采用六甲时捷径，天禽寄宫及月家年家自动命名关闭',
      appliesTo: ['时家奇门', '九星', '八门', '伏吟', '反吟'],
      sources: [
        '《遁甲符应经》“子来加子为伏吟”“子来加午为反吟，门符冲对皆是”',
        '《奇门宝鉴御定》“凡星门各有定位，若加临于对冲之位，即为反吟”“凡星门各加临于本位，即为伏吟”',
        '《奇门遁甲统宗》天蓬加天蓬为伏吟、天蓬到天英为反吟及八门反覆同例',
        '《遁甲符应经》又称六甲之时门符皆伏吟，而《奇门宝鉴御定》明确反对把六甲序直接作为伏吟',
        '当前时家九宫值符星、值使门实际落宫复算记录',
      ],
      promptText:
        '星门伏吟/反吟位置规则：仅在时家盘面中，值符星实际回到自身本位或落到本位对冲宫时，分别记录星伏吟或星反吟；值使门实际回本宫或落本宫对冲宫时，分别记录门伏吟或门反吟。《遁甲符应经》把六甲之时门符皆伏吟，《奇门宝鉴御定》则明确反对这种时辰捷径，当前只采用两书共同的实际位置定义，不因六甲时自动补名。天禽居中五且转盘涉及寄宫版本，当前关闭天禽伏反吟自动命名；月家、年家也不得套用。所有命中只证明位置，不继承原典兵事、财货、动静或通用吉凶断语',
      limitation: RULE_SOURCE_LIMITATION,
    },
    {
      key: 'rule:qimen:door-controls-palace-structure',
      status: '已声明',
      category: '门宫五行结构规则',
      rule: '八门五行克所在宫五行时只记录“门克宫”中性结构；门迫、宫迫及迫制和义格名称因古籍用词倒置和层级差异失败关闭',
      appliesTo: ['时家奇门', '月家奇门', '年家奇门', '八门', '九宫五行'],
      sources: [
        '《遁甲符应经》“宫制其门，是为门迫；门制其宫，是为宫迫”',
        '《奇门宝鉴御定》同段歌诀称“宫制其门不为迫，门制其宫是迫推”，解释又称“宫制其门曰门迫，门制其宫曰宫迫”',
        '《奇门旨归》“八门克宫为迫，宫克八门为制”',
        '《奇门遁甲秘笈大全》一处称“门尅宫为迫，宫尅门为制”，另一处又采用门迫、宫迫对称名称',
        '当前八门本属五行与九宫本属五行逐宫复算记录',
      ],
      promptText:
        '门宫五行结构规则：八门与九宫共 72 种组合逐一按标准五行生克复算；只有门五行克所在宫五行时记录“门克宫”中性事实。由于《遁甲符应经》《奇门宝鉴御定》《奇门旨归》《奇门遁甲秘笈大全》对“迫”是否专指门克宫虽大体一致，但“门迫/宫迫”的名称存在倒置，“迫制和义格”的命名层级也不统一，因此不得把该事实自动改名为门迫、宫迫或迫制和义格。时家、月家、年家均只保留这项基础五行关系，不继承主客、吉凶、行动或方位断语',
      limitation: RULE_SOURCE_LIMITATION,
    },
    {
      ...
      sources: [
        ...
        '《奇门遁甲秘笈大全》旬空六旬表同列上述六组',
        '《奇门遁甲秘笈大全》驿马起例明确写作“寅午戌日马在申”等日马四组',
        '当前排盘完整年、月、日、时四柱原始事实',
      ],
      promptText:
    ...
    },
    {
      ...
      status: '已声明',
      category: '经典格局审核边界',
      rule: '正式入口输出已逐条校勘的十一项天地盘固定格，以及仅限时家的伏干格、飞干格、岁格、格勃、天乙飞宫格、天乙伏宫格、严格相佐、三奇升殿、三诈、天假/严格地假/鬼假与玉女守门中性结构；守户、九遁、三奇得使、天辅时、五合时名称、天网/天网四张、三奇/时干入墓及三奇受制已确认存在版本或名称定义冲突，三奇会甲缺少可核验闭合原文，符使同宫仅有起局机制与位置事实支持；其余三奇、人假、物假、神假、五假扩展、月格、时格、普通勃格及其他旧格局在版本、条件和适用情境闭环前失败关闭',
      ...
      sources: [
        ...
        '《奇门遁甲统宗》《奇门旨归》《奇门法窍》《遁甲演义》天乙飞宫、天乙伏宫条件互证结果',
        '《遁甲演义》《奇门遁甲秘笈大全》《奇门旨归》《奇门法窍》相佐共同交集及守户名称条件冲突审核结果',
        '《奇门宝鉴御定》符使同起于甲遁宫、随后分别加临的排布机制审核结果',
        ...
        '当前九宫门、星、神、天地盘干、已审核时家时旬空与五行关系原始事实',
      ],
      promptText:
        '经典格局审核边界：当前把青龙返首、飞鸟跌穴、青龙逃走、白虎猖狂、朱雀投江、螣蛇跃蹻、荧入太白、太白入荧、大格、刑格、小格十一项固定格，以及仅限时家复算的伏干格、飞干格、岁格、六庚值符临丙格勃、值符临地庚的天乙飞宫格、天盘庚临地值符旬首遁干的天乙伏宫格、值符临地丙的严格相佐、乙奇到震三/丙奇到离九/丁奇到兑七三项升殿位置结构、真诈/重诈/休诈三项奇门神同宫结构、天假/严格地假/鬼假三项条件一致结构和六个固定时柱且值使临地盘丁的玉女守门结构视为已校勘命中。甲申旬值符与六庚身份重合时不命名天乙飞伏宫；相佐不扩大到地盘乙、丁。所有时家上下文格只登记中性结构，不采用互有差异的现实断语。守户存在值使临地丁、时干临地丁及守星/守门/守户异名冲突；符使同宫原文只支持值符、值使同起于旬首甲遁宫后分别加临的排布机制，同宫时保留两个位置索引而不另造格名；月格存在“月干/月朔干”差异，时格存在“本时干/仅三奇/庚值符管十时”差异，普通勃格存在“丙临年月日时干/丙加值符庚”等冲突，天辅时存在主表与《太白阴经》同条异说冲突，五合时存在与天辅时混称，天网/天网四张存在六癸时、天盘癸落宫、值符临地癸及特定局数等定义冲突，三奇/时干入墓存在宫位表、时柱表与日干前提混用，三奇受制存在位置表与门干囚死复合条件差异，三奇会甲缺少可核验闭合原文，均继续失败关闭；九遁、三奇得使、三奇游六仪、人假、物假、神假、五假扩展、三奇入墓/受制/会甲、符使同宫、守户及其他未审核旧规则也不得从原始盘面自动补算为既定格局或现实结论',
      limitation: RULE_SOURCE_LIMITATION,
    },
    {
      ...
      status: '已声明',
      category: '节令背景事实层级',
      rule: '节令背景只保留实际节气、日干旺相休囚死、精确月相与建除名称；上中下元只采用正式定局结果；四柱关系不自动命名条件未闭合的半合、拱局或二支三刑',
      appliesTo: ['节令背景', '上中下元', '月相', '建除', '四柱干支关系'],
      sources: [
        '《奇门法窍》“此法以甲、己两将为符头，如值子午卯酉为上元，寅申巳亥为中元，辰戌丑未为下元”',
        '《奇门法窍》“甲己临仲上元天，临孟之时作中元，临季下元局日定……五日一候为一元”',
        '《太白阴经》“五日六十时为一元”',
        '《钦定协纪辨方书》引《考原》“三合者，取生、旺、墓三者以合局也”及四组三合全表',
        '《奇门遁甲秘笈大全》“寅刑巳，巳刑申，申刑寅，丑刑戌，戌刑未，未刑丑……辰、午、酉、亥为自刑”',
        '《滴天髓阐微》“二支会局者，以相贴为妙，逢冲即破，他字间之，亦遥隔无力，须天干领出可用”',
        '节气历表、太阳视黄经、历法八相与日月黄经差独立计算结果',
      ],
      promptText:
        '节令背景事实层级：上中下元只引用当前盘按甲己符头或所选置闰法形成的正式定局结果，不得把交节后自然日机械分成三个五日段另造一套三元。月相保留历法八相名称与日月黄经差证据，不把蛾眉月、盈凸月等粗并成上弦或满月。建除只保留名称，不据此补造脱离具体事项的固定吉凶、宜忌或行动建议。四柱关系只把六合、六冲、相害、子卯相刑、自刑、三合或三刑完整三支齐见、天干五合及已登记天干冲作为中性结构；三合三支齐见不等于成局或合化。半合、拱局还涉及相邻、冲破、透干等条件，寅巳申与丑戌未的任意二支也不足以在当前无版本入口中直接命名，均保留原始四柱供明确口径后继续推算',
      limitation: RULE_SOURCE_LIMITATION,
    },
    {
      ...
      status: '已声明',
      category: '组合规则版本',
      rule: '八门余气固定采用《奇门遁甲统宗》五态月令版；十干迫制原文限定年命推占并要求比较双方旺衰，通用盘条件不足时不自动命名',
      appliesTo: ['八门余气', '十干迫制失败关闭边界', '已校勘组合'],
      sources: [
        '《奇门遁甲统宗》卷十二“当时者为旺，我生者为相，我克者为休，克我者为囚，生我者为废”',
        '《奇门遁甲统宗》卷十二“甲乙金宫、丙丁坎内、戊己惧杜伤、庚辛离上、壬癸生死方”',
        '《奇门遁甲统宗》卷十二“此以十干迫制推占年命之法”及各组五行旺衰比较条件',
        '《奇门宝鉴御定》八门气应另有逐节旺、绝、胎、没、死、囚、休、废八态版，当前不混用',
      ],
      promptText:
        '已校勘组合版本：八门余气只用《奇门遁甲统宗》五态月令版，不混用《奇门宝鉴御定》逐节八态版。十干迫制属于推占年命之法，除固定落宫外还须确认年命宫并比较干、宫双方旺衰；当前通用盘缺少这些前提，不得仅凭奇仪落宫受克自动命名。盘面保留天盘干、宫位与五行原始事实，待明确年命和旺衰口径后继续推算',
      limitation: RULE_SOURCE_LIMITATION,
    },
    {
  ...
  const counterSummaryFact: QimenCounterSummaryFact = {
    ...
    promptText: counterEvidenceFacts.length
      ...
      : scope === 'hour'
        ? '当前时家九宫未见时旬空位置限制；这不构成现实吉凶判断'
        : `当前${scopeLabel}旬空适用层级未闭合，未自动推算；不得写成“不空”`,
