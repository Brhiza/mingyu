...
export function analyzeQimenEvidence(input: QimenData): QimenEvidenceAnalysis {
  ...
  const ruleSourceFacts: QimenRuleSourceFact[] = [
    ...
    },
    {
      ...
      status: '已声明',
      category: '经典格局审核边界',
      rule: '正式入口输出已逐条校勘的十一项天地盘固定格，以及仅限时家的伏干格、飞干格、岁格、格勃、天乙飞宫格、天乙伏宫格、三奇升殿、三诈、天假/严格地假/鬼假与玉女守门中性结构；九遁、三奇得使、天辅时、五合时名称、天网/天网四张、三奇/时干入墓及三奇受制已确认存在版本定义冲突，三奇会甲缺少可核验闭合原文；其余三奇、人假、物假、神假、五假扩展、符使、月格、时格、普通勃格及其他旧格局在版本、条件和适用情境闭环前失败关闭',
      ...
      promptText:
        '经典格局审核边界：当前把青龙返首、飞鸟跌穴、青龙逃走、白虎猖狂、朱雀投江、螣蛇跃蹻、荧入太白、太白入荧、大格、刑格、小格十一项固定格，以及仅限时家复算的伏干格、飞干格、岁格、六庚值符临丙格勃、值符临地庚的天乙飞宫格、天盘庚临地值符旬首遁干的天乙伏宫格、乙奇到震三/丙奇到离九/丁奇到兑七三项升殿位置结构、真诈/重诈/休诈三项奇门神同宫结构、天假/严格地假/鬼假三项条件一致结构和六个固定时柱且值使临地盘丁的玉女守门结构视为已校勘命中。甲申旬值符与六庚身份重合时不命名天乙飞伏宫。所有时家上下文格只登记中性结构，不采用互有差异的现实断语。月格存在“月干/月朔干”差异，时格存在“本时干/仅三奇/庚值符管十时”差异，普通勃格存在“丙临年月日时干/丙加值符庚”等冲突，天辅时存在主表与《太白阴经》同条异说冲突，五合时存在与天辅时混称，天网/天网四张存在六癸时、天盘癸落宫、值符临地癸及特定局数等定义冲突，三奇/时干入墓存在宫位表、时柱表与日干前提混用，三奇受制存在位置表与门干囚死复合条件差异，三奇会甲缺少可核验闭合原文，均继续失败关闭；九遁、三奇得使、三奇游六仪、人假、物假、神假、五假扩展、三奇入墓/受制/会甲、符使同宫、相佐守户及其他未审核旧规则也不得从原始盘面自动补算为既定格局或现实结论',
      limitation: RULE_SOURCE_LIMITATION,
    },
    {
