...
export function analyzeQimenEvidence(input: QimenData): QimenEvidenceAnalysis {
  ...
  const ruleSourceFacts: QimenRuleSourceFact[] = [
    ...
    },
    {
      ...
      promptText:
        '天地盘干规则：八十一种组合完整保留天干、五行生克与标准五合结构；仅青龙返首、飞鸟跌穴、青龙逃走、白虎猖狂、朱雀投江、螣蛇跃蹻、荧入太白、太白入荧、大格、刑格、小格十一项按已校勘固定格输出，其余七十项不得单凭二元组合补造传统名称或现实断语；时家另按独立上下文规则核验伏干格、飞干格、岁格、六庚值符临丙格勃、三奇升殿与三诈位置结构',
      limitation: RULE_SOURCE_LIMITATION,
    },
    {
    ...
    },
    {
      ...
      status: '已声明',
      category: '经典格局审核边界',
      rule: '正式入口输出已逐条校勘的十一项天地盘固定格，以及仅限时家的伏干格、飞干格、岁格、格勃、三奇升殿与三诈中性结构；九遁、其余三奇、五假、符使、月格、时格、普通勃格及其他旧格局在版本、条件和适用情境闭环前失败关闭',
      ...
      promptText:
        '经典格局审核边界：当前把青龙返首、飞鸟跌穴、青龙逃走、白虎猖狂、朱雀投江、螣蛇跃蹻、荧入太白、太白入荧、大格、刑格、小格十一项固定格，以及仅限时家复算的伏干格、飞干格、岁格、六庚值符临丙格勃、乙奇到震三/丙奇到离九/丁奇到兑七三项升殿位置结构和真诈/重诈/休诈三项奇门神同宫结构视为已校勘命中。所有时家上下文格只登记中性结构，不采用互有差异的现实断语。月格存在“月干/月朔干”差异，时格存在“本时干/仅三奇/庚值符管十时”差异，普通勃格存在“丙临年月日时干/丙加值符庚”等冲突，继续失败关闭；九遁、三奇得使、三奇游六仪、五假、三奇入墓/受制/会甲、符使同宫、相佐守户、天乙飞伏宫、天辅时、玉女守门、门宫相生等旧规则也不得从原始盘面自动补算为既定格局或现实结论',
      limitation: RULE_SOURCE_LIMITATION,
    },
    {
