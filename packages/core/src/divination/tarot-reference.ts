...
const TAROT_KEYWORDS: Record<string, string> = {
  愚者: '新开始,冒险,纯真',
  魔术师: '意志力,创造,技能',
  女祭司: '直觉,神秘,内在智慧',
  女皇: '丰饶,母性,创造力',
...
export function validateTarotReferenceData(): void {
  ...
  const keywordNames = Object.keys(TAROT_KEYWORDS);
  const majorNames = tarotCards.filter((card) => card.type === '大阿卡纳').map((card) => card.name);
  if (
    ...
    throw new Error('塔罗牌组必须按 1-78 完整登记且牌号、牌名不重复');
  }
  if (
    keywordNames.length !== 78 ||
    keywordNames.some((name) => !names.includes(name)) ||
    names.some((name) => !TAROT_KEYWORDS[name])
  ) {
    throw new Error('塔罗78张牌必须逐张完整登记关键词');
  }
  const reversedNames = Object.keys(MAJOR_REVERSED_MEANINGS);
  if (
    reversedNames.length !== 22 ||
    reversedNames.some((name) => !majorNames.includes(name)) ||
    majorNames.some((name) => !MAJOR_REVERSED_MEANINGS[name])
  ) {
    throw new Error('塔罗22张大阿卡纳必须逐张完整登记逆位解释');
  }
}
...
export function getCardKeywords(cardName: string): string {
  const keywords = TAROT_KEYWORDS[cardName];
  if (!keywords) throw new Error(`未知的塔罗牌名: ${cardName}`);
  return keywords;
}
