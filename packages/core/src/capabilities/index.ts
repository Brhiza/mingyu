...
const systems: SystemCapability[] = [
  ...
  },
  {
    ...
    outputs: [
      ...
      '定局方法',
      '符头与超神接气',
      '值符、值使、日干、时干与已校勘格局位置索引（不自动选定用神宫）',
      '门星神干证据',
      '天地盘干81种结构事实',
      '11项已校勘固定十干格',
      '时家完整上下文下已校勘的伏干格、飞干格、岁格、六庚值符临丙格勃中性结构事实',
      '时家值符临地庚的天乙飞宫格与天盘庚临地值符旬首遁干的天乙伏宫格中性结构事实（甲申旬身份重合关闭）',
      '时家值符临地盘丙的严格相佐中性结构事实（地盘乙丁不扩大，守户名称条件冲突关闭）',
      '值符和值使当前同宫时分别保留两个位置索引（符使同宫独立格名及现实断语关闭）',
      ...
      '时家按完整日柱日支起日马并完成六十日柱穷举；时马、月马、年马及年月家外推关闭边界',
  ...
  },
  {
    ...
    outputs: ['起课时间与干支', '农历月日', '时辰及序号', '历法边界说明', '版本待校说明'],
    supports: {
      seed: false,
      customRandomSource: false,
      replay: false,
  ...
  },
  {
    ...
    outputs: ['起课方式', '时间与四柱', '用户原始数字', '随机轨迹', '规则待校状态', '结构化证据'],
    supports: randomSupports,
    notes: [
      '具体底本、版本、原文位置和页码未闭合前，只保留起课方式、时间、四柱、用户原始数字或随机轨迹，不自动计算金口诀课盘。',
      '证据入口只信任原始起课资料，旧缓存中的地分、四位、发用、五动三动与解释字段不会进入新结果；随机轨迹缺失或矛盾时明确拒绝。',
    ],
  },
  {
    id: 'liuren',
  ...
  },
  {
    ...
    outputs: ['项目内部牌号', '牌名', '抽牌顺序', '牌位', '正逆位', '牌义待校状态', '结构化证据'],
    supports: randomSupports,
    notes: [
      '逐牌关键词、正逆位牌义、元素、牌阶主题和组合规则在具体牌组版本与原文映射闭合前失败关闭。',
      '项目内部牌号、抽牌与正逆位轨迹只用于复算过程，不证明牌义、预测有效性或现实结果。',
    ],
  },
  {
    id: 'lenormand',
    ...
    outputs: ['项目内部牌号', '牌名', '抽牌顺序', '牌位', '牌义待校状态', '结构化证据'],
    supports: randomSupports,
    notes: [
      '关键词、单牌牌义、固定组合、相邻合读和布局解释在具体牌组版本与原文映射闭合前失败关闭。',
      '项目内部牌号、牌名与抽牌轨迹只用于复算过程，不证明牌义、预测有效性或现实结果。',
    ],
  },
  {
    id: 'ssgw',
  ...
  },
  {
    ...
    notes: [
      'year 与 yearGanZhi 至少明确提供一项，不回退系统当前年份；同时提供时必须对应同一个立春年。',
