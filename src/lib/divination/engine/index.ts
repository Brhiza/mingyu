...
export function buildDivinationPrompt(
  ...
  const outputRequirementText = isTarot
    ...
    : isLenormand
      ...
      : isSsgw
        ? '只说明可核验的签号、抽取轨迹及签谱与掷筊规则待校边界；不得补造签谱内容、模拟掷筊或给出由签号推导的判断。'
        : isTaiyi
          ...
          : isXiaoliuren
            ...
            : isJinkoujue
              ...
              : isLiuren
                ? '先核对具体类神底本版本、事项类别与参与者角色、完整类神取用规则和已指定类神对象是否同时齐全；缺少任一项时，只说明可复算课传事实与资料缺口，不生成现实演变、成败、时机或行动建议。'
                : isQimen
                  ? '先核对具体底本版本、事项角色、完整取用规则和已指定用神对象是否同时齐全；缺少任一项时，只说明可复算盘面事实与资料缺口，不生成方位、应期、现实结果或行动建议。'
                  : isAlmanac
                    ? '按可用候选、条件候选和慎用候选分组说明；同组按日期先后，不生成首选、备选或唯一最佳结论；只说明已列分组依据、时段、现实刚性条件与资料限制，不扩写执行建议。'
                    : isAstrolabe
