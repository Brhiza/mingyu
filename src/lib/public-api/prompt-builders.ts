...
const ZIWEI_SCHOOL_GUIDANCE: Record<ZiweiSchool, string> = {
  sanhe:
    '紫微解读侧重点：三合派。以命身宫位、主星庙旺、对宫与三方四正为主线，四化作牵引；排盘信息中列明的基础安星口径为基准。',
  feixing:
    '紫微解读侧重点：飞星派。以盘面已提供的生年四化、当前运限四化、自化与飞化落宫为主线，三方四正作会照辅证；排盘信息中列明的基础安星口径为基准。',
  sihua:
    '紫微解读侧重点：四化派。以盘面已提供的生年四化定位、运限四化触发与禄权科忌落宫为主线，星曜庙旺与三方用于解释四化条件；排盘信息中列明的基础安星口径为基准。',
};
...
export function buildPublicZiweiPromptForRuntime(params: {
  ...
  return [promptWithSchool, '', `【任务】\n${buildPublicZiweiTaskText()}`].join('\n');
}
