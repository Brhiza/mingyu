...
function getFlowChanges(previous: QizhengResult, current: QizhengResult): string {
  const facts = (result: QizhengResult) => {
    ...
    return [
      ...
      ...(limits
        ? [
            `虚岁${limits.nominalAge}、${limits.direction}、大限宫位未定、小限${limits.currentMinorLimit.signBranch}宫${limits.currentMinorLimit.palace}、太岁入${limits.annualPalace.signBranch}宫${limits.annualPalace.palace}`,
          ]
        : []),
