...
function buildFortuneUsageBoundaryText(scope: FortuneSelectionContext['scope']) {
  const boundaryMap: Record<FortuneSelectionContext['scope'], string> = {
    dayun: '解读范围：重点判断这步大运的十年阶段主题，应期只到大运层级。',
    year: '解读范围：重点判断这一年的年度触发，应期只到流年层级。',
    month: '解读范围：重点判断这个节气月窗口，应期只到流月层级。',
...
function buildBaziFortuneInterpretationRules(scope: FortuneSelectionContext['scope']): string {
  ...
  return [
    ...
    '写应期时，先说明上层背景，再说明当前层级的触发证据；只在【分析对象】明确的时间层级内给出窗口，不延伸到更细日期。',
  ].join('\n');
}
...
function buildBaziOutputRequirementText(kind: 'single' | 'fallback' = 'single') {
  const firstLine =
    kind === 'fallback'
      ? '先直接回答【问题】，再补完整依据、触发条件与建议。'
      : '先直接回答【问题】，再按“结论总览、命局主线、关键证据、反证限制、应期触发、现实建议”展开。';

  return [
    firstLine,
    '结论总览要直接说明倾向、强弱和条件，不要只写“需要综合判断”。',
    '命局主线要交代旺衰、格局、调候、用忌、十神和宫位如何共同形成判断，不要只列术语。',
    '关键证据至少分成主证、辅证、反证或限制；有【分析对象】时必须说明对应年限如何触发，本命范围只判断长期趋势。',
    '应期触发要区分长期底色、阶段运限、年度触发、月日短期窗口；只在【分析对象】明确的层级内写条件，不硬给唯一日期。',
    '证据只能支持倾向时就写倾向，不要为了给结论而延伸新的盘面事实。',
...
function getCompatibilityOutputRequirement(compatType?: CompatType): string {
  ...
  return [
    ...
    '关系主轴要说明双方长期相处底色和当前问题倾向，不要只给吉凶标签。',
    '互补与冲突都要写明双方盘面主证、辅证、反证或限制；证据不足处单独说明。',
    '触发条件要区分本命长期模式与当前岁运阶段，不能把一时冲合作为永久结论。',
    '最后给出可执行建议：适合推进、观察还是暂缓，以及沟通、合作或边界管理的具体做法。',
  ].join('\n');
}
