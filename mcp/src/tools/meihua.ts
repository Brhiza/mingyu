...
export function registerMeihuaTool(server: McpServer) {
  server.registerTool(
    ...
    {
      description:
        '梅花易数起卦：支持时间起卦、数字起卦、随机起卦，timeTrigram 作为兼容旧参数按时间起卦计算，生成主卦、互卦、变卦、体用生克分析、现场资料边界及应期资料',
  ...
  server.registerTool(
    ...
    {
      description:
        '梅花易数起卦并生成结构化 AI 解读提示词：一次调用返回卦盘数据（含主互变卦、体用生克、现场资料边界与应期资料）和可直接复制给 AI 的提示词',
