...
| `metaphysics_tieban`  | 铁板排盘     | 返回太玄数、考刻、先后天卦和公开条文框架                                       |
| `tieban_prompt`       | 铁板提示词   | 铁板排盘并返回可直接用于 AI 解读的提示词                                       |
| `metaphysics_qizheng` | 七政四余     | 返回七政四余宿度、十二宫、庙旺和神煞                                           |
...
8. 用户明确要求八宅、生肖犯太岁、太乙、铁板或七政四余时，使用对应的 `*_prompt` 工具；只要原始排盘则使用 `metaphysics_*`。
