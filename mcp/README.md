...
| `bazi_time_sensitivity`      | 时间敏感性   | 按出生记录误差前后重排候选盘，返回稳定柱位、变化柱位与结构化证据               |
...
2. 用户明确只看单人八字时调用 `bazi_prompt`；出生时间接近边界或记录可能有误差时调用 `bazi_time_sensitivity`；询问两人婚恋、合作或亲属互动时调用 `bazi_compatibility_prompt`；长期或完整阶段分析优先传 `baziFortuneScope: "full"`。
