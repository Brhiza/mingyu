...
| `foundation_wuxing`          | 五行分析     | 统计天干地支五行分布，可选逐项展开藏干，不设置强度权重                                                           |
...
| `bazi_compatibility`         | 八字双盘     | 返回双方命盘、四柱交叉关系、双向十神、喜忌资料覆盖或资料缺口与证据包                                             |
...
| `divine_qimen`               | 奇门遁甲排盘 | 返回九宫原始盘、值符值使、位置索引、宫间五行、旬空马星与 11 项已校勘固定格；不自动生成用神、吉方、避方或应期结论 |
| `qimen_prompt`               | 奇门提示词   | 返回不含数字评分的位置索引、九宫事实及可直接用于 AI 解读的结构化提示词；由 AI 按问题继续选择用神                 |
...
| `metaphysics_zodiac`         | 生肖流年     | 返回值冲刑害破、三合六合三会、五行生克方向及解释边界，不生成利弊或行动结论                                       |
| `zodiac_prompt`              | 生肖提示词   | 返回固定关系事实和可供 AI 结合问题继续推算的提示词，不把三合六合改写成现实贵人                                   |
| `metaphysics_taiyi`          | 太乙年计     | 按积年与阳遁七十二局立成返回年计式盘；月、日、时计完成古籍历法链校勘后再开放                                     |
| `taiyi_prompt`               | 太乙提示词   | 太乙排盘并返回可直接用于 AI 解读的提示词                                                                         |
| `metaphysics_qizheng`        | 七政四余     | 返回十一星、真实距星宿界、5项命身十二职宫规则、55组星对夹角、8项生年起例与审计边界                               |
| `qizheng_prompt`             | 七政提示词   | 七政四余排盘并返回可直接用于 AI 解读的结构化提示词                                                               |

七政四余的七政、罗睺、计都和月孛采用现代天文位置，二十八宿按 28 颗真实距星在目标日期的黄经划界；紫炁采用《七政算内篇》古法均速模型。黄道宫支、命宫、十二职宫和命主按《张果星宗》校勘，身宫按《五行精纪》《灵台经》取太阴所在宫；结果逐星同时返回热带黄道星座和十二支宫，避免混用两套序号。真太阳时只校正命宫所用生时与可能跨日的传统年界，不改变现代天体计算时刻，也不另行改算身宫。十一星的 55 组无序星对完整返回实际最小夹角；天乙昼贵、玉堂夜贵分开按生年干起例，另有 6 项生年支起例，春节与立春年界分歧时不自动选边，目标支不代表盘面命中或吉凶。固定容许度吊照与简化庙旺表因缺少闭合原典依据而不自动判定。使用 IANA 历史时区启用真太阳时时，必须提供 `standardMeridian`，系统不会把法定钟表偏移猜成标准经线。
...
8. 用户明确要求八宅、生肖与流年固定关系、太乙或七政四余时，使用对应的 `*_prompt` 工具；只要原始资料则使用 `metaphysics_*`。
...
八字结果的 `generation` 保存规范化出生输入、完整神煞口径、中国夏令时开关和生成时间。命盘、合盘、岁运证据和提示词只凭该来源与上述原始命限选择重新计算，不采信旧四柱、格局、神煞、大运或派生岁运上下文；来源缺失、夹带、矛盾或非法时明确拒绝。
...
返回结果会包含 `seasonality`（节气三元、节气五行、月相、建除十二神、四柱干支互动）和 `patternCombos`（固定文献条件可闭合的门宫、主客、方位与时机组合规则）。经典格、空亡、门迫、伏吟反吟和驿马分别保留在原字段中，不会被二次拼成新格名或叠加吉凶等级；提示词工具会把这些基础事实和已校勘规则分别作为解读证据。
...
星盘结果的 `generation` 保存规范化出生输入和生成时间；公开证据、提示词、摘要、行运与高级时限只凭该来源重建，不采信旧星体、宫位、相位、光照、摘要、时区诊断或顶层时间。缺少合法来源的旧结果会失败关闭。

本命相位会对 24 个点位的 276 组无序点对逐一核验 10 种相位；行运会对 10 个行运星体与全部 24 个本命点逐一核验五种主要相位，取样使用目标地点当日历史时区。次限、太阳弧和太阳返照同样完整保留当前定义与容许度内的全部命中项。结果只给出实际夹角、精确角、偏差和采用容许度，不使用派生强度门槛，不分级、不按强度或偏差排名删减、不截断。

西占双盘工具使用 `person1`、`person2` 分别传入上述星盘参数。结果会穷举全部所选点对，完整返回所有命中跨盘相位的实际夹角、精确角、偏差、采用容许度和全部双向落宫；这些只属于可复核盘面事实，不分级、不排序、不截断，也不派生关系强弱、匹配率、吉凶百分比或必然结果。双盘 `generation` 另保存双方本命来源、完整点位选择、五种相位容许度、落宫开关和生成时间；重建与提示词忽略旧双盘派生结果，来源缺失或非法时明确拒绝。
